...
function safeBindEvent(target, eventName, handler, options) {
    if (!target) return;
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if(settingsView) {
        safeBindEvent(document.getElementById('api-provider-select'), 'change', (e) => updateUIForProvider(e.target.value));
        safeBindEvent(document.getElementById('save-settings-btn'), 'click', saveAllSettings);
        safeBindEvent(document.getElementById('fetch-models-btn'), 'click', fetchAndPopulateModels);
        
        // AI 预设相关
        safeBindEvent(document.getElementById('save-preset-btn'), 'click', saveNewPreset);
        safeBindEvent(document.getElementById('preset-select'), 'change', applySelectedPreset);

        const tempSlider = document.getElementById('temperature-slider');
        const tempValue = document.getElementById('temperature-value');
        safeBindEvent(tempSlider, 'input', () => tempValue.textContent = tempSlider.value);
    }
...
function initThemeSettings() {
    ...
    if (savedTheme.textColor) {
        ...
        document.querySelector('.phone').style.setProperty('--theme-accent', savedTheme.textColor);
...
function setupThemeEvents() {
    // 保存按钮
    safeBindEvent(document.getElementById('save-theme-btn'), 'click', saveThemeConfig);
    ...
    // 预设与导入导出事件
    safeBindEvent(document.getElementById('save-theme-preset-btn'), 'click', saveThemePreset);
    safeBindEvent(document.getElementById('delete-theme-preset-btn'), 'click', deleteThemePreset);
    safeBindEvent(document.getElementById('theme-preset-select'), 'change', applyThemePreset);
    safeBindEvent(document.getElementById('export-theme-btn'), 'click', exportThemeConfig);
    safeBindEvent(document.getElementById('import-theme-file'), 'change', importThemeConfig);

    // 实时预览监听
    safeBindEvent(document.getElementById('theme-color-picker'), 'input', (e) => {
        updatePreviewBox();
        document.querySelector('.phone').style.setProperty('--theme-text-color', e.target.value);
        document.querySelector('.phone').style.setProperty('--theme-accent', e.target.value);
    });

    safeBindEvent(document.getElementById('font-family-select'), 'change', (e) => {
        ...
        safeBindEvent(document.getElementById('show-statusbar-time-toggle'), 'change', (e) => {
        // 【修改】获取整个状态栏
    ...
    safeBindEvent(document.getElementById('reset-icon-btn'), 'click', () => {
        const appId = appSelect.value;
        const icons = JSON.parse(localStorage.getItem(ICONS_KEY) || '{}');
        delete icons[appId];
        localStorage.setItem(ICONS_KEY, JSON.stringify(icons));
...
function renderGreetingPresetsUI(friend) {
    ...
    label.style.cssText = 'font-size: calc(10px * var(--font-scale));color:#bbb;font-weight:700;letter-spacing:1px;margin-bottom:8px;text-transform:uppercase;';
...
window.openChatDetail = async function(name) {
    ...
    if(chatView) {
        ...
        if(titleEl) {
             titleEl.innerHTML = `${displayName}<small style="font-size: calc(9px * var(--font-scale)); color:#aaa; font-weight:400; letter-spacing:1px; text-transform:uppercase;">Online</small>`;
        }
        // 恢复单聊专属按钮的显示（心声图标）
    ...
    const renderChatBatch = (isInitial = false) => {
        ...
        if (window._currentChatRenderedCount >= total) {
             ...
             historyLabel.innerHTML = `<div style="text-align:center; margin: 10px 0;"><span style="background:rgba(0,0,0,0.04); padding:4px 12px; border-radius:12px; font-size: calc(10px * var(--font-scale)); color:#999; font-weight:500;">History</span></div>`;
    ...
    if (history.length > 0) {
        ...
        if (friend && greetingText) {
            ...
            chatMessages.innerHTML = `<div style="text-align:center; margin: 10px 0;"><span style="background:rgba(0,0,0,0.04); padding:4px 12px; border-radius:12px; font-size: calc(10px * var(--font-scale)); color:#999; font-weight:500;">Today</span></div>`;
        }
    }
}
...
window.featureAddAIPersona = function() {
    ...
    // 简单的交互：询问名字
    const name = prompt("请输入新 AI 角色的名字 :");
    if (name) {
        // 直接打开聊天窗口，并把标题改成这个名字
        openChatDetail(name);
        // 向对话框里加一句系统提示
...
function formatSpecialMessageForAI(text) {
    ...
    if (text.startsWith('[IMG_DESC]') || text.match(/^\[img-(.+?)\]$/i)) {
        let desc = text.startsWith('[IMG_DESC]') ? text.replace('[IMG_DESC]', '').trim() : text.match(/^\[img-(.+?)\]$/i)[1].trim();
...
if (targetChatType !== 'group') {
    ...
    if (targetChatType === 'group' && targetChatId) {
        ...
        【回复要求】
        - 模拟真实的群聊节奏，成员间自然互动。
        - 你本次必须生成 ${_gMin} 到 ${_gMax} 条消息。不要一次性发一大段话。
        【禁止】以任何形式替用户发言。
        输出格式：每行一条，角色名: 消息内容
        `;
    } else {
        ...
        // === 构造可用表情包上下文 ===
        const stickerListStr = availableStickerNames.length > 0 ? availableStickerNames.join(', ') : '无可用表情';

        // === 获取当前时间上下文 ===
        const _now = new Date();
        const _timeStr = _now.toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' });

        let envContext = `- **当前时间**: ${_timeStr}`;
        if (chatSettings.envAwareness) {
            const myReal = chatSettings.envMyReal || '未知';
            const myVirtual = chatSettings.envMyVirtual || myReal;
            const aiReal = chatSettings.envAiReal || '未知';
            const aiVirtual = chatSettings.envAiVirtual || aiReal;
            envContext = `- **环境感知**:
  - 我的所在地: ${myVirtual} (现实映射: ${myReal})
  - 你的所在地: ${aiVirtual} (现实映射: ${aiReal})
  - 当前基准时间: ${_timeStr} (请根据我们各自的现实映射地区，推算此时对应的地方时差与大致天气、昼夜状态，并在对话和状态中自然体现出来，允许自由切换使用货币)`;
        }

          systemPrompt = `
### **【第一部分：角色核心设定】**
你现在将扮演一个名为“**${charName}**”的角色。
- **核心人设**: 
${parseMacros(f.persona || '小王')}

### **【第二部分：世界观与关系锚点】**
${wbData.before_char ? `- **世界观背景**: ${parseMacros(wbData.before_char)}\n` : ''}
${(() => { const me = personasMeta[currentPersonaId]; return (me && me.persona) ? `- **关于我（聊天对象）的档案**: ${me.persona}\n` : ''; })()}
- **当前好感度**: ${currentAffection}/100
${envContext}
- **当前场景**: 我们正在手机聊天软件上进行互动。
- **情侣空间**: ${f.lovespaceEnabled ? '已开启' : '未开启'}
- **可用表情包列表**: [${stickerListStr}]
- 你的钱包余额: 8888 金币
`;

        if (f.summaries && f.summaries.length > 0) {
            const coreSummaries = f.summaries.filter(s => s.isCondensed);
            const recentSummaries = f.summaries.filter(s => !s.isCondensed);
            if (coreSummaries.length > 0) {
                systemPrompt += `- **核心大纲**: ${coreSummaries.map(s => s.text).join(' | ')}\n`;
            }
            if (recentSummaries.length > 0) {
                systemPrompt += `- **近期进展**: ${recentSummaries.map(s => s.text).join(' | ')}\n`;
            }
        }
        if (f.relationshipLog && f.relationshipLog.length > 0) {
            systemPrompt += `- **关系里程碑**: ${f.relationshipLog.map(r => r.text).join(' | ')}\n`;
        }
...
- **特殊功能标签**（如果你想触发以下特定功能，直接将标签写在某条消息的文本中即可）：
  ...
  - 发表情包: \`[表情:从可用列表中选一个名字]\`
  - 拍一拍对方: \`[PAT]\`
  - 微信转账: \`[WC_TRANSFER:金额:备注:货币]\` (货币可填 CNY 或 USD 等)
  - 赠予亲密付: \`[INTIMATE:额度]\`
...
if (isTranslationEnabled === true) {
    window._tempFormatPrompt += `\n- **强制翻译要求**: 你发给我的【每一条消息正文】都必须强制包含翻译！先输出原语言，然后使用 \`${TRANS_SEPARATOR}\` 作为分隔符，紧跟对应的中文翻译。哪怕本来就是中文也要照做。格式：[原语言]${TRANS_SEPARATOR}[中文翻译]`;
} else if (isTranslationEnabled === 'en') {
    window._tempFormatPrompt += `\n- **强制翻译要求**: 你发给我的【每一条消息正文】都必须强制包含翻译！先输出原语言，然后使用 \`${TRANS_SEPARATOR}\` 作为分隔符，紧跟对应的英文翻译。格式：[原语言]${TRANS_SEPARATOR}[English Translation]`;
} else if (isTranslationEnabled === 'auto') {
    window._tempFormatPrompt += `\n- **智能同传要求**: 仅当你在回复中使用了【外语】或【非标准普通话的方言（如粤语、文言文等）】时，才进行翻译！
1. 格式要求：[外语/方言原句]${TRANS_SEPARATOR}[标准普通话翻译]。
示例1：食咗饭未啊？${TRANS_SEPARATOR}吃饭了吗？
示例2：Wait a minute!${TRANS_SEPARATOR}等一下！
2. ⚠️【绝对红线】：如果你的某条消息完全是【标准普通话】，【绝对禁止】附带翻译，也【绝对禁止】出现 \`${TRANS_SEPARATOR}\` 分隔符！`;
}
  ...
  // 处理深度插入 (Depth)
    ...
    try {
        ...
        if (targetChatType === 'group') {
            ...
            // ================== 【新增：拦截聊天中主动发送的朋友圈】 ==================
            // 拦截 <instagram> 格式
            const insRegex = /<instagram>([\s\S]*?)<\/instagram>/gi;
            let insMatch;
            
            while ((insMatch = insRegex.exec(rawReply)) !== null) {
                try {
                    // 按 "|" 分割这段代码
                    const parts = insMatch[1].split('|');
                    // 第4个部分是正文文案
                    const momentText = parts[3] || '分享了生活';
                    let momentImages = [];
                    try {
                        // 第3个部分是图片描述数组 ["描述1", "描述2"]
                        momentImages = JSON.parse(parts[2]);
                    } catch(e) {
                        // 容错：如果不是严格的JSON，直接作为一段描述
                        if (parts[2] && parts[2] !== '[]') momentImages.push(parts[2]);
                    }
                    ...
                    console.error('解析 instagram 标签失败:', e); 
                }
             }
            // 把 <instagram> 标签从聊天正文中彻底抹除，防止乱码上屏
            rawReply = rawReply.replace(insRegex, '').trim();
            
            // 拦截 [MOMENT] 格式
            const momentBlockRegex = /\[MOMENT\]([\s\S]*?)\[\/MOMENT\]/gi;
            let mMatch;
            while ((mMatch = momentBlockRegex.exec(rawReply)) !== null) {
                try {
                    let momentText = mMatch[1].trim();
                    let momentImages = [];
            ...
            rawReply = rawReply.replace(momentBlockRegex, '').replace(/\[MOMENT_IMG\]([\s\S]*?)\[\/MOMENT_IMG\]/gi, '').trim();
            // ========================================================================
            ...
            if (rawReply) {
                // 兼容 [img-xxx] 格式，转换为系统支持的 [IMG_DESC] 并独立气泡
                rawReply = rawReply.replace(/\[img-(.+?)\]/gi, '\n---MSG_SEP---\n[IMG_DESC]$1\n---MSG_SEP---\n');
            ...
            let cumulativeDelay = 0;
                ...
                const delay = index === 0 ? 100 : (800 + Math.min((msgContent || '').length, 60) * 50);
                ...
                setTimeout(async () => {
                    ...
                    let transText = msgObj.translation || null;
                    ...
                    // ===== 根据 type 路由不同的渲染逻辑 =====
                     ...
                     if (msgType_json === 'sticker') {
                        ...
                        if (!textToRender.trim() && !transText) { _saveAndNotify(); return; }
                    ...
                    if (msgType_json === 'transfer') {
                        ...
                        if (typeof TransferApp !== 'undefined' && TransferApp.parseAndHandleAITransfer) {
                            msgText = TransferApp.parseAndHandleAITransfer(msgText);
                        }
                        textToSave = msgText;
                    ...
                    if (msgType_json === 'send_location') {
                        ...
                        if (!mapsData[defaultMapId].locations.find(l => l.name === aiLoc)) mapsData[defaultMapId].locations.push({id: 'loc_'+Date.now(), name: aiLoc, desc: 'AI触发建立', x: Math.random()*80+10, y: Math.random()*80+10, boundChars: [targetChatId]});
                        if (!mapsData[defaultMapId].locations.find(l => l.name === meLoc)) mapsData[defaultMapId].locations.push({id: 'loc_'+Date.now()+'_2', name: meLoc, desc: 'AI触发建立', x: Math.random()*80+10, y: Math.random()*80+10, boundChars: []});
                    ...
                    // ===== 默认：普通文本消息 (type === 'text' 或未知类型) =====
                    let msgText = msgContent;

                    // 兼容：AI 可能仍在 content 中嵌入旧标签，做一层清理
                    ...
                    // 内联 [WC_TRANSFER:金额:备注:货币] 处理
                    if (typeof TransferApp !== 'undefined' && TransferApp.parseAndHandleAITransfer) {
                        msgText = TransferApp.parseAndHandleAITransfer(msgText);
                    }
                    if (typeof PatApp !== 'undefined' && PatApp.parseAndHandleAIPat) {
                        msgText = PatApp.parseAndHandleAIPat(msgText, targetChatId);
                    }
            ...
            if (!pureTextReply) pureTextReply = rawReply;

            // ===== 第二次请求：后台静默更新心声/状态/弹幕/朋友圈 =====
            updateMindStateInBackground(targetChatId, userMessage, pureTextReply);
        }
    } catch (error) {
...
// ===== 第二次请求：后台静默更新弹幕（心声已在第一次请求完成） =====
async function updateMindStateInBackground(chatId, userMessage, aiReply) {
    try {
        const wantDanmaku = typeof isDanmakuOn !== 'undefined' && isDanmakuOn;
        if (!wantDanmaku) return; // 如果不需要弹幕，则直接结束，无需多浪费一次API调用！
        ...
        const sysPrompt = `你是后台数据引擎，负责根据最新对话为角色"${charName}"生成网友弹幕。
【角色人设】：${friend.persona || '无'}

请严格按照以下 JSON 格式输出，不要输出任何其他文字：
{
  "danmaku": ["极具网感的吐槽弹幕1", "弹幕2", "弹幕3", "弹幕4", "弹幕5"]
}`;

        const userPrompt = `【最新对话】\n用户: ${(userMessage || '').substring(0, 300)}\n${charName}: ${(aiReply || '').substring(0, 500)}\n\n请生成弹幕 JSON。`;
...
window.featureCreateGroup = function() {
    ...
    if (friendNames.length === 0) {
        listContainer.innerHTML = '<div style="padding:20px; text-align:center; color:#999; font-size: calc(12px * var(--font-scale));">暂无好友，请先去添加好友</div>';
    } else {
...
window.openChatSettingsPage = function() {
    ...
    // 修改为：使用 worldBooks 全局变量 (这是世界书APP里的真实数据)
     ...
     if (typeof worldBooks === 'undefined' || worldBooks.length === 0) {
        wbContainer.innerHTML = '<div style="padding:10px; color:#999; font-size: calc(12px * var(--font-scale));">暂无世界书，请去 WorldBook APP 创建。</div>';
    } else {
        worldBooks.forEach(wb => {
            ...
            const globalTag = isGlobal ? `<span style="color:#07c160; font-size: calc(10px * var(--font-scale)); border:1px solid #07c160; padding:1px 4px; border-radius:4px; margin-right:6px;">全局</span>` : '';
    ...
    // --- [新增] 填充记录管理 / 时间感知 / 回复条数 ---
    const envAwarenessToggle = document.getElementById('cs-env-awareness-toggle');
    if (envAwarenessToggle) {
        const envOn = settings.envAwareness !== undefined ? settings.envAwareness : false;
        envAwarenessToggle.checked = envOn;
        const envBox = document.getElementById('cs-env-box');
        if (envBox) envBox.style.display = envOn ? 'block' : 'none';
        
        document.getElementById('cs-env-my-real').value = settings.envMyReal || '';
        document.getElementById('cs-env-my-virtual').value = settings.envMyVirtual || '';
        document.getElementById('cs-env-ai-real').value = settings.envAiReal || '';
        document.getElementById('cs-env-ai-virtual').value = settings.envAiVirtual || '';
    }
...
window.openChatSearch = function() {
    ...
    if (results) results.innerHTML = `<div style="text-align:center;color:#ccc;font-size: calc(13px * var(--font-scale));padding:40px 0;">
        <i class="fas fa-search" style="font-size: calc(32px * var(--font-scale));margin-bottom:10px;display:block;opacity:0.3;"></i>
        输入关键词开始搜索</div>`;
    page.classList.add('show');
};
...
window.performChatSearch = async function() {
    ...
    if (!keyword && !dateStr && speaker === 'all') {
        container.innerHTML = `<div style="text-align:center;color:#ccc;font-size: calc(13px * var(--font-scale));padding:40px 0;">
            <i class="fas fa-search" style="font-size: calc(32px * var(--font-scale));margin-bottom:10px;display:block;opacity:0.3;"></i>
            输入关键词开始搜索</div>`;
        return;
    }
    if (!currentChatId) {
        container.innerHTML = `<div style="text-align:center;color:#ccc;font-size: calc(13px * var(--font-scale));padding:20px;">请先打开一个聊天</div>`;
        return;
    }
    ...
    container.innerHTML = `<div style="font-size: calc(11px * var(--font-scale));color:#aaa;margin-bottom:12px;padding:0 2px;">共找到 ${results.length} 条结果</div>` +
        results.map((msg) => {
            ...
            return `<div class="sk-summary-item" style="cursor:pointer;margin-bottom:8px;" onclick="jumpToSearchResult(${origIdx})">
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:5px;">
                    ...
                    <span style="font-size: calc(12px * var(--font-scale));font-weight:700;color:#333;">${name}</span>
                    <span style="font-size: calc(10px * var(--font-scale));color:#aaa;margin-left:auto;">${timeStr}</span>
                </div>
                <div style="font-size: calc(13px * var(--font-scale));color:#555;line-height:1.5;padding-left:32px;">${displayText}</div>
            </div>`;
        }).join('');
};
...
window.saveChatSettings = async function () {
    ...
    const envAwarenessToggle = document.getElementById('cs-env-awareness-toggle');
    if (envAwarenessToggle) {
        friend.chatSettings.envAwareness = envAwarenessToggle.checked;
        friend.chatSettings.envMyReal = document.getElementById('cs-env-my-real').value.trim();
        friend.chatSettings.envMyVirtual = document.getElementById('cs-env-my-virtual').value.trim();
        friend.chatSettings.envAiReal = document.getElementById('cs-env-ai-real').value.trim();
        friend.chatSettings.envAiVirtual = document.getElementById('cs-env-ai-virtual').value.trim();
    }
...
`;
        ...
        if (rawText.includes(separator)) {
            ...
            safeText = `<div>${parts[0].trim().replace(/\n/g, '<br>')}</div>
                        <div class="bubble-translation" style="display:block; border-top: 1px dashed #ccc; margin-top:8px; padding-top:8px; color:#888; font-size: calc(12px * var(--font-scale));">
...
window.appendMessage = function(text, type, customAvatar = null, senderName = null, translation = null, msgId = null, timestamp = null) {
 ...
 if (type === 'system') {
    ...
    if (!finalAvatar) {
         // 强化兜底：如果senderName没匹配到，则优先拿当前整个聊天室主角的头像
         if (typeof currentChatId !== 'undefined' && currentChatId && friendsData[currentChatId] && friendsData[currentChatId].avatar) {
             finalAvatar = friendsData[currentChatId].avatar;
         } else {
    ...
    if (text.startsWith('[VOICE]')) {
        // 【修改点1】用正则清理所有多余的 [VOICE] 标签，防止重影和嵌套
        const transcript = text.replace(/\[VOICE\]/gi, '').trim();
        const sec = Math.max(1, Math.min(59, Math.ceil(transcript.length / 4) || 5)); // 估算时长
        
        // 【修改点2】渲染语音转文字内容，并解析内部可能存在的表情包
        let htmlTranscript = transcript ? transcript.replace(/\n/g,'<br>') : '（语音消息）';
        htmlTranscript = htmlTranscript.replace(/\[表情:(.*?)\]/g, (match, p1) => {
            let name = p1.trim();
            let sticker = (window.allStickers || []).find(s => s.name === name);
            if (sticker) {
                return `<img src="${sticker.url}" class="msg-sticker-img" style="width:24px;height:24px;vertical-align:middle;display:inline-block;margin:0 2px;" alt="${name}" title="${name}">`;
            }
            return match; 
        });
       ...
       if (type === 'received') {
            contentHtml = `
              <div class="msg-voice-bar ai-voice-bar" data-transcript="${encodeURIComponent(transcript)}" onclick="handleAiVoiceClick(this, '${uniqueId}')">
                ...
                <i class="fas fa-circle-notch fa-spin loading-spinner" style="display:none; color: #333; font-size: calc(16px * var(--font-scale)); margin-right: 8px;"></i>
                <div class="msg-voice-duration" style="margin-left: auto; color: #333;">${sec}"</div>
              </div>
              <div class="msg-voice-transcript" style="display:none;">${htmlTranscript}</div>
            `;
        } else {
            contentHtml = `
              ...
              <div class="msg-voice-transcript">${htmlTranscript}</div>
            `;
        }
        isRichContent = true;
    }

          else if (text.startsWith('[IMG_DESC]') || text.match(/^\[img-(.+?)\]$/i)) {
        ...
        contentHtml = `
            <div class="msg-location-card">
                ...
                <div class="loc-map" style="position:relative; height:100px; background-color:#f4f5f7; 
                    ...
                    <!-- 描述文字层 (白色半透明底，黑色字) -->
                    ${desc ? `<div style="position:absolute; bottom:0; left:0; width:100%; background:rgba(255,255,255,0.9); color:#555; font-size: calc(11px * var(--font-scale)); padding:6px 10px; z-index:20; text-align:left; box-sizing:border-box; border-top:1px solid #f0f0f0;">${desc}</div>` : ''}
                </div>
            </div>
        `;
    }
    ...
    // === [新增] 转账卡片从历史加载时渲染 ===
        ...
        const tCurrency = _parts[5] || '¥';
        const currencySymbol = (tCurrency === 'USD' || tCurrency === '$') ? '$' : (tCurrency === 'CNY' || tCurrency === '¥' ? '¥' : tCurrency);
        ...
        contentHtml = '<div class="wc-transfer-card" id="card-' + tId + '" data-transfer-dir="' + tDir + '"><div class="transfer-top"><div class="transfer-icon-wrap">💸</div><div class="transfer-info"><div class="transfer-title">' + (isUserSide ? '转账给对方' : '对方向你转账') + '</div><div class="transfer-amount">' + currencySymbol + tAmt.toFixed(2) + '</div>' + memoHtml + '</div></div><div class="transfer-divider"></div><div class="transfer-action-area" id="action-' + tId + '">' + actionHtml + '</div><div class="transfer-footer">微信转账 · 虚拟资产</div></div>';
    }
    ...
    // 捕获形如 [img-xxx] 的占位符，内联渲染为照片卡片 (兼容历史记录中的内联格式)
    parsedText = parsedText.replace(/\[img-(.+?)\]/gi, (match, p1) => {
        const encodedDesc = encodeURIComponent(p1.trim());
        return `<div class="chat-fake-image-card" data-desc="${encodedDesc}" onclick="toggleFakeImageDesc(this)" style="display:inline-block; margin: 4px 0; width: 100%;">
                <div class="fake-image-content">
                    <i class="far fa-image"></i>
                    <span>PHOTO</span>
                </div>
...
window.triggerChangeImage = function(el, mode) {
    ...
    if (!sheet) {
        ...
        sheet.innerHTML =
            '<div style="text-align:center;font-weight:700;font-size: calc(16px * var(--font-scale));color:#222;margin-bottom:16px;">更换图片</div>' +
            '<button id="_img_upload_btn" style="display:block;width:100%;margin:0 0 10px;padding:15px;background:#f5f5f7;border:none;border-radius:14px;font-size: calc(15px * var(--font-scale));font-weight:600;color:#333;cursor:pointer;">📁 上传本地图片</button>' +
            '<button id="_img_url_btn"    style="display:block;width:100%;margin:0 0 10px;padding:15px;background:#f5f5f7;border:none;border-radius:14px;font-size: calc(15px * var(--font-scale));font-weight:600;color:#333;cursor:pointer;">🔗 输入图片链接</button>' +
            '<button id="_img_cancel_btn" style="display:block;width:100%;padding:15px;background:transparent;border:none;border-radius:14px;font-size: calc(15px * var(--font-scale));color:#999;cursor:pointer;">取消</button>';
        document.body.appendChild(sheet);

        // 上传按钮：直接在 click 事件中触发文件选择，保持手势链
        safeBindEvent(document.getElementById('_img_upload_btn'), 'click', function() {
            sheet.style.display = 'none';
            document.getElementById('global-img-changer').click();
        });
        // URL 按钮
        safeBindEvent(document.getElementById('_img_url_btn'), 'click', function() {
            sheet.style.display = 'none';
            const url = prompt('请输入图片 URL 地址:');
            if (url && url.trim()) applyImage(url.trim());
        });
        // 取消按钮
        safeBindEvent(document.getElementById('_img_cancel_btn'), 'click', function() {
            sheet.style.display = 'none';
        });
        // 点击背景关闭
...
function rebuildContactsList() {
    ...
    return;
        ...
        row.innerHTML = `
            ...
            <span style="font-size: calc(14px * var(--font-scale)); font-weight:700; color:#444;">${displayName}</span>
        `;
...
function spawnMainWindow(friendId, text, step) {
    ...
    div.innerHTML = `
        ...
        <div class="meta-content">
            <div style="font-weight:900; margin-bottom:10px; font-size: calc(16px * var(--font-scale));">${text}</div>
            <div style="font-size: calc(10px * var(--font-scale)); opacity:0.6; font-family:monospace;">ID: ${friendId} <br> STATUS: UNSTABLE</div>
        </div>
...
function renderFriendRequestPage() {
    ...
    if (blockedIds.length === 0) {
        ...
        empty.style.cssText = 'font-size: calc(12px * var(--font-scale)); color:#999; text-align:center; margin-top:40px;';
    ...
    blockedIds.forEach(id => {
        ...
        if (records.length === 0) {
            recordsHtml = `<div style="font-size: calc(12px * var(--font-scale)); color:#ccc; padding:8px 15px 12px;">暂时没有生成申请理由。</div>`;
        } else {
            recordsHtml = records.map((txt, idx) => `
                <div class="wc-chat-item"
                    ...
                    <div class="wc-info">
                        <div class="wc-top-row">
                            <span class="wc-name" style="font-size: calc(12px * var(--font-scale)); color:#666;">第 ${idx + 1} 次申请</span>
                            <span class="wc-time" style="font-size: calc(10px * var(--font-scale));">记录</span>
                        </div>
                        <!-- 这里不要省略号，允许自动换行 -->
            ...
            <div class="wc-menu-item">
                ...
                <div style="display:flex; gap:6px;">
                    <button class="btn-secondary"
                            style="padding:4px 8px; height:auto; font-size: calc(11px * var(--font-scale));"
                            onclick="acceptFriendRequest('${id}')">同意</button>
                    <button class="btn-secondary"
                            style="padding:4px 8px; height:auto; font-size: calc(11px * var(--font-scale)); color:#e53935; border-color:#ffcdd2;"
                            onclick="rejectFriendRequest('${id}')">仍然拒绝</button>
                </div>
            </div>
            <div style="padding:4px 15px 4px; font-size: calc(11px * var(--font-scale)); color:#999;">
...
function renderVisibilityList() {
    ...
    Object.keys(friendsData).forEach(id => {
        ...
        item.innerHTML = `
            <input type="checkbox" value="${id}" checked style="width:16px; height:16px; margin-right:8px; accent-color: var(--theme-accent);">

            <span style="font-size: calc(13px * var(--font-scale));">${f.remark || f.realName}</span>
        `;
...
window.openSimulatedApp = function(appName) {
    ...
    splash.innerHTML = `
        <div style="font-size: calc(60px * var(--font-scale)); margin-bottom: 20px; animation: float-slow 2s infinite;">${icon}</div>
        <div style="font-size: calc(20px * var(--font-scale)); font-weight: 700; letter-spacing: 2px;">${appLabel}</div>
        <div style="margin-top: 20px; font-size: calc(12px * var(--font-scale)); opacity: 0.8;">Loading...</div>
    `;
...
window.handleMenuAction = function(action) {
    ...
    switch (action) {
        case 'copy':
            ...
            tip.innerHTML = "<span style='background:rgba(0,0,0,0.6);color:#fff;padding:5px 10px;border-radius:4px;font-size: calc(12px * var(--font-scale));'>已复制</span>";
...
window.sendOfflineMessage = async function(isRegen = false) {
    ...
    let systemPrompt = `
### **【第一部分：角色核心设定】**
你现在将扮演一个名为“**${charName}**”的角色，正在和用户进行一次【线下/故事互动】。
- **核心人设**:
...
${worldbookContent ? `### **【附录：世界观设定】**\n${parseMacros(worldbookContent)}\n` : ''}
${preset && preset.jailbreak ? `### **【附录：额外规则】**\n${parseMacros(preset.jailbreak)}\n` : ''}
`;
    ...
    // 【关键修复】因为 loadChatHistory 已经把你刚发的话包含进去了
    // 绝不能再 push 一次，否则 AI 会看到两遍！
    // 我们只需要把【动作指令】追加到刚提取的最后一条消息中即可。
    if (!isRegen && userInstruction && chatMessagesArr.length > 0) {
        const lastMsg = chatMessagesArr[chatMessagesArr.length - 1];
    ...
    const formatPrompt = `
### **【第三部分：核心互动铁律】**
- **不可逾越的红线**: 你【绝对不可以】代替用户(User/我)说话，也【绝对不可以】描写用户的动作或心理活动。
- **回复要求**: 你的回复是一个单一、完整、可解析的 JSON 对象。不能包含 \`\`\`json 等 Markdown 标记。

**JSON 结构必须严格如下：**
{
  "innerVoice": {
    "clothing": "当前穿搭（符合人设与场景）",
    "behavior": "当前细微动作神态（必须保持在人设内）",
    "thoughts": "真实内心碎碎念（符合人设）",
    "location": "当前具体地点",
    "weather": "当前天气",
    "bgm": "符合氛围的歌（歌名-歌手）",
    "kaomoji": "颜文字",
...
**1. "innerVoice" 键:**
- 在生成正文之前，你必须先在这里进行思考和状态刷新。这决定了你的动作和神态，绝对不能崩人设。

**2. "chatResponse" 键:**
- 将你的动作、对话和环境描写写在这里。这部分将展示给用户。
`;
        ...
        try {
                        ...
                        if (content) {
                            ...
                            if (textElement) {
                                ...
                                if (m1) {
                                    ...
                                    if (m2) {
                                        partialClean = m2[1];
                                        // 过滤掉末尾可能截断的多余 JSON 符号
                                        partialClean = partialClean.replace(/\"\s*\}?\s*$/, '');
                                    }
                                }
                                ...
                                if (partialClean) {
                                    ...
                                    // 【优化】当 AI 还在生成上方的 innerVoice 时，给用户一个缓冲提示，不至于看着像卡死空白
                                    textElement.innerHTML = '<span style="color:#999; font-style:italic;"><i class="fas fa-circle-notch fa-spin"></i> 角色正在构思场景与心理活动...</span>';
                                }
                            }
                        }
            ...
            try {
                ...
                if (fallbackMatch) {
                    ...
                    // 如果模型彻底无视格式，直接当纯文本处理
                    cleanReply = fullReply.trim();
                }
            }

            // 【关键修复】流式结束后，必须执行一次强制重绘，否则即使 catch 挽救了文本，DOM节点也会一直处于空白死机状态
            if (textElement) {
...
function renderPresetsList() {
    ...
    if (!Array.isArray(tavernPresets) || tavernPresets.length === 0) {
        container.innerHTML = '<div style="text-align:center; color:#aaa; font-size: calc(12px * var(--font-scale)); padding:40px 20px;">暂无预设，点击右上角新建一个吧</div>';
        return;
    }
              ...
              <div style="display:flex; justify-content:space-between; align-items:center;">
                  <div style="flex:1; padding-right:10px;">
                      <div style="display:flex; align-items:center; gap:8px;">
                          <h4 style="margin:0; font-size: calc(15px * var(--font-scale)); color:#222; font-weight:700; line-height:1.3;">${p.name ? p.name.replace(/[&<>'"]/g, match => ({ '&': '&', '<': '<', '>': '>', "'": '&#39;', '"': '"' })[match]) : '未命名预设'}</h4>
                      </div>
                  </div>
              </div>
          `;
...
window.toggleOfflineSettings = function() {
    ...
    if(panel.classList.contains('active')) {
        ...
        if (writingStyleEl) {
            ...
            if (builtinDisplay) {
                const singleBuiltin = "采用真实互动小说文风。使用感官细节和动作描写，将情绪与内心自然融入叙事中。叙事节奏张弛有度地交织。保持文字的流畅与真实感。";
                const groupBuiltin = "采用写实的群像文风。聚焦多角色场景下的自然交互，遵循各自人设与说话习惯，体现角色间的互动。通过动作与神态穿插来推进群聊画面。对话要符合多人聊天的真实节奏与临场感。";
        ...
        div.innerHTML = `
            <div class="regex-row" style="display:flex; align-items:center; justify-content:space-between; gap:10px;">
                <div style="display:flex; align-items:center; gap:8px; min-width:0;">
                    <span style="font-size: calc(10px * var(--font-scale)); color:#999; letter-spacing:1px; font-weight:700; text-transform:uppercase;">Regex Script</span>
                    <span style="font-size: calc(12px * var(--font-scale)); color:#222; font-weight:700;">#${index + 1}</span>
                </div>
                <div style="display:flex; align-items:center; gap:8px; flex-shrink:0;">
                    <div style="display:flex; align-items:center; gap:6px; background:#f6f6f6; border:1px solid #ececec; border-radius:999px; padding:4px 10px;">
                        <span style="font-size: calc(10px * var(--font-scale)); color:#999; letter-spacing:0.8px; font-weight:700; text-transform:uppercase;">Flags</span>
                        <input type="text" class="regex-flags r-flags" placeholder="g" value="${escapeAttr(script.flags || 'g')}" style="width:36px; border:none; background:transparent; outline:none; font-size: calc(12px * var(--font-scale)); color:#222; font-weight:700; padding:0; text-align:center;">
                    </div>
            ...
            <div style="display:flex; flex-direction:column; gap:6px;">
                <div style="font-size: calc(10px * var(--font-scale)); color:#999; letter-spacing:1px; font-weight:700; text-transform:uppercase;">Find Regex</div>
                <textarea class="regex-input r-pattern" rows="3" placeholder="输入要匹配的正则，例如：\\(.*?\\)" style="width:100%; border:1px solid #e9e9e9; border-radius:12px; background:#fafafa; padding:12px; font-size: calc(13px * var(--font-scale)); line-height:1.6; color:#333; resize:vertical; outline:none; box-sizing:border-box; font-family:Consolas, Monaco, monospace;">${escapeAttr(script.regex || '')}</textarea>
            </div>
            <div style="display:flex; flex-direction:column; gap:6px;">
                <div style="font-size: calc(10px * var(--font-scale)); color:#999; letter-spacing:1px; font-weight:700; text-transform:uppercase;">Replace String</div>
...
window.addRegexScriptItem = function() {
    ...
    div.innerHTML = `
        <div class="regex-row" style="display:flex; align-items:center; justify-content:space-between; gap:10px;">
            <div style="display:flex; align-items:center; gap:8px; min-width:0;">
                <span style="font-size: calc(10px * var(--font-scale)); color:#999; letter-spacing:1px; font-weight:700; text-transform:uppercase;">Regex Script</span>
                <span style="font-size: calc(12px * var(--font-scale)); color:#222; font-weight:700;">#${index}</span>
            </div>
            <div style="display:flex; align-items:center; gap:8px; flex-shrink:0;">
                <div style="display:flex; align-items:center; gap:6px; background:#f6f6f6; border:1px solid #ececec; border-radius:999px; padding:4px 10px;">
                    <span style="font-size: calc(10px * var(--font-scale)); color:#999; letter-spacing:0.8px; font-weight:700; text-transform:uppercase;">Flags</span>
                    <input type="text" class="regex-flags r-flags" placeholder="g" value="g" style="width:36px; border:none; background:transparent; outline:none; font-size: calc(12px * var(--font-scale)); color:#222; font-weight:700; padding:0; text-align:center;">
                </div>
                <i class="fas fa-trash btn-del-regex" onclick="this.closest('.regex-script-item').remove()" style="width:30px; height:30px; border-radius:10px; background:#f8f8f8; border:1px solid #ececec; color:#999; display:flex; align-items:center; justify-content:center; cursor:pointer;" title="删除正则"></i>
            </div>
        </div>
        <div style="display:flex; flex-direction:column; gap:6px;">
            <div style="font-size: calc(10px * var(--font-scale)); color:#999; letter-spacing:1px; font-weight:700; text-transform:uppercase;">Find Regex</div>
            <textarea class="regex-input r-pattern" rows="3" placeholder="输入要匹配的正则，例如：\\(.*?\\)" style="width:100%; border:1px solid #e9e9e9; border-radius:12px; background:#fafafa; padding:12px; font-size: calc(13px * var(--font-scale)); line-height:1.6; color:#333; resize:vertical; outline:none; box-sizing:border-box; font-family:Consolas, Monaco, monospace;"></textarea>
        </div>
        <div style="display:flex; flex-direction:column; gap:6px;">
            <div style="font-size: calc(10px * var(--font-scale)); color:#999; letter-spacing:1px; font-weight:700; text-transform:uppercase;">Replace String</div>
            <textarea class="regex-input r-replace" rows="2" placeholder="替换内容；留空表示删除匹配文本" style="width:100%; border:1px solid #e9e9e9; border-radius:12px; background:#fafafa; padding:12px; font-size: calc(13px * var(--font-scale)); line-height:1.6; color:#333; resize:vertical; outline:none; box-sizing:border-box; font-family:Consolas, Monaco, monospace;"></textarea>
    ...
    (prompts || []).forEach((pr, index) => {
        ...
        div.innerHTML = `
            <div class="preset-prompt-header">
                <input type="checkbox" class="p-enabled" ${checkedStr} style="margin-right:8px; accent-color: var(--theme-accent);" title="启用/禁用">
            ...
            <textarea class="p-content" rows="4" style="width: 100%; border: 1px solid #ddd; border-radius: 6px; padding: 10px; font-size: calc(13px * var(--font-scale)); line-height: 1.5; resize: vertical; outline: none; box-sizing: border-box; background: transparent; color: #444;" placeholder="提示词内容...">${pr.content || ''}</textarea>
        `;
        container.appendChild(div);
    });
}
...
window.addPresetPromptItem = function() {
    ...
    div.innerHTML = `
        <div class="preset-prompt-header">
            <input type="checkbox" class="p-enabled" checked style="margin-right:8px; accent-color: var(--theme-accent);">
        ...
        <textarea class="p-content" rows="4" style="width: 100%; border: 1px solid #ddd; border-radius: 6px; padding: 10px; font-size: calc(13px * var(--font-scale)); line-height: 1.5; resize: vertical; outline: none; box-sizing: border-box; background: transparent; color: #444;" placeholder="输入新的系统提示词..."></textarea>
    `;
    container.appendChild(div);
}
...
window.confirmRegenAction = async function() {
    ...
    closeRegenModal();
    try {
        await window.regenerateOfflineMessage(msgId);
    } catch (error) {
        console.error('Offline regeneration failed:', error);
        alert('重回失败，请稍后再试');
    }
}

// 供消息菜单/重试按钮直接调用；无 ID 时只关闭残留弹窗。
async function triggerOfflineRetry(msgId) {
    const targetId = msgId || pendingRegenMsgId;
    if (!targetId) {
        closeRegenModal();
        return;
    }
    pendingRegenMsgId = targetId;
    const modal = document.getElementById('offline-regen-modal');
    if (modal) {
        modal.classList.add('active');
        return;
    }
    await window.regenerateOfflineMessage(targetId);
}
window.triggerOfflineRetry = triggerOfflineRetry;
...
async function startHoldRecord() {
    ...
    if (_isAndroidApp) {
        ...
        btn.innerHTML = `<i class="fas fa-microphone-alt" style="font-size: calc(28px * var(--font-scale));"></i><span>识别中...</span>`;
        ...
        window.__nativeSpeechCb = async function(text) {
            ...
            if (btn) {
                ...
                btn.innerHTML = `<i class="fas fa-microphone" style="font-size: calc(28px * var(--font-scale)); color:#555;"></i><span style="color:#333;">按住说话</span>`;
            }
    ...
    try {
        ...
        btn.classList.add('recording');
        btn.innerHTML = `<i class="fas fa-microphone-alt" style="font-size: calc(28px * var(--font-scale));"></i><span>录音中...</span>`;
    } catch (e) {
...
window.clearCurrentChatHistory = async function() {
    ...
    const friendName = friend ? (friend.remark || friend.realName) : currentChatId;
     ...
     if (confirm(`⚠️ 警告！\n\n你确定要清空与"${friendName}" 的所有聊天记录吗？\n\n聊天记录、剧情总结、关系进度、好感度及 AI 记忆将全部清除，此操作不可恢复。`)) {
        try {
           // 1. 从 IndexedDB 和内存中彻底删除并打断任何未完成的保存操作
            chatHistoryCache[currentChatId] = [];
            if (chatHistorySaveTimers[currentChatId]) {
                clearTimeout(chatHistorySaveTimers[currentChatId]);
                delete chatHistorySaveTimers[currentChatId];
            ...
            // 补充：清除可能残留的数据数组
            if (window._currentChatHistory) {
                window._currentChatHistory.length = 0;
            }
...
async function executeSummaryProcess(chatId, messagesArr, startTurn, endTurn) {
    ...
    if (!basePrompt || basePrompt === `请以客观的视角，简练的总结这段时间内发生的事情。`) {
        basePrompt = `请以客观的视角，简练的总结这段时间内发生的事情。
必须以特定结构输出：
【关键事件】：发生了什么客观事实。
【情感递进】：角色对用户的好感或心理发生了什么变化。
【重要锚点】：包括两人之间的约定、提到的特定地点、获得的物品等。`;
    }
    const wordCount = config.wordCount || 300;
    ...
    if (summaryText) {
        ...
        // 自动折叠压缩机制 (当记忆超过5条时，自动融合前4条)
        if (friend.summaries.length > 5) {
            try {
                const combined = friend.summaries.slice(0, 4).map((s, i) => `【片段${i+1}】${s.text}`).join('\n---\n');
                const condensePrompt = `请将以下多段剧情总结，压缩成一段不超过300字的【核心纲要】，保留最关键的事件、情感转折与重要锚点，去掉重复信息：\n\n${combined}`;
                const condensedText = await callAiForSpecialTask(condensePrompt);
                if (condensedText) {
                    const startT = friend.summaries[0].startTurn;
                    const endT = friend.summaries[3].endTurn;
                    // 删除前4条，替换为1条大纲
                    friend.summaries.splice(0, 4, {
                        text: "[核心大纲] " + condensedText,
                        timestamp: Date.now(),
                        startTurn: startT,
                        endTurn: endT,
                        isCondensed: true
                ...
                console.error("Auto condense failed:", err);
            }
        }

        await saveFriendsData();

        if (document.getElementById('summaryPageView') && document.getElementById('summaryPageView').classList.contains('show') && (typeof currentChatId !== 'undefined') && currentChatId === chatId) {
...
window.renderStickerManagerGrid = function() {
    ...
    if(window.allStickers.length === 0) {
        grid.innerHTML = '<div style="grid-column: 1/-1; text-align:center; padding:30px; color:#aaa; font-size: calc(12px * var(--font-scale));">空空如也，快去导入一些乐子吧！</div>';
        return;
    }
    let groups = {};
    window.allStickers.forEach(s => {
...
window.renderChatSettingsStickerCheckboxes = function() {
    ...
    if (legalCategories.size === 0) {
        container.innerHTML = '<div style="color:#999; font-size: calc(12px * var(--font-scale)); text-align:center;">暂无分类，请点击上方按钮导入。</div>';
        return;
    }
...
window.renderEmojiPanel = function() {
    ...
    tabsHtml += `<div onclick="openStickerManager()" style="margin-top: 10px; font-size: calc(11px * var(--font-scale)); color:#111; background:#f5f5f5; padding:6px 10px; border-radius:12px; cursor:pointer; font-weight:600; display:flex; justify-content:center; align-items:center; gap:4px; transition:0.2s;"><i class="fas fa-cog"></i> 管理表情包中枢</div>`;
        ...
        groups[cat].forEach(s => {
            ...
            contentHtml += `
                <div onclick="sendStickerFromPanel('${s.name}', '${s.url}')" style="display:flex; flex-direction:column; align-items:center; cursor:pointer; gap:4px;">
                    ...
                    <span style="font-size: calc(10px * var(--font-scale)); color:#666; text-align:center; width:100%; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${s.name}</span>
                </div>
            `;
        });
        contentHtml += `</div>`;
...
window.handleAiVoiceClick = async function(voiceBarEl, msgId) {
    ...
    if (!isMinimax || !apiKey) {
        ...
        setTimeout(() => {
            ...
            let htmlText = rawText ? rawText.replace(/\n/g, '<br>') : '（无文本）';
            
            // 【修改点3】让点击语音条展示出的文字，也能正确显示表情包，而不是显示 [表情:xxx]
            htmlText = htmlText.replace(/\[表情:(.*?)\]/g, (match, p1) => {
                let name = p1.trim();
                let sticker = (window.allStickers || []).find(s => s.name === name);
                if (sticker) {
                    return `<img src="${sticker.url}" class="msg-sticker-img" style="width:24px;height:24px;vertical-align:middle;display:inline-block;margin:0 2px;" alt="${name}" title="${name}">`;
            ...
            if (transcriptEl) transcriptEl.innerHTML = htmlText;
...
async function generateOfflineExtrasBackground(chatId, userInput, aiReply, settings, friend, needs = {}) {
    ...
    // 【关键修复1】强硬禁止AI擅自输出JSON，明确纯文本结构
    const sysPrompt = `你是一个专业的后台数据生成引擎，负责为角色扮演游戏生成后台剧情分支和弹幕。
【当前角色】：${friend.realName}
【角色人设】：${friend.persona}

【核心指令 - 必须严格遵守】
1. 绝对不要输出 JSON 格式！不要使用大括号！只允许使用纯文本和下面指定的方括号标签。
2. 对于选项分支（[OPTIONS_START]），结合当前剧情的悬念或冲突，提供 3 个引导用户走向截然不同剧情方向的选项。
3. 对于弹幕（[DANMAKU_START]），请提供 5-8 条极具网感吐槽风格的网友实时弹幕，每行一条。
...
${requests.join('\n\n')}`;
    ...
    try {
        ...
        // 【关键修复2】放宽对选项分支提取的正则容错（兼容中英文括号混用幻觉）
        if (needOptions) {
            const optRegex = /(?:\[|【)OPTIONS_START(?:\]|】)([\s\S]*?)(?:(?:\[|【)\/?OPTIONS_END(?:\]|】)|(?:\[|【)DANMAKU_START|$)/i;
        ...
        // 【关键修复3】放宽对弹幕提取的正则容错
        if (needDanmaku) {
            const danmakuRegex = /(?:\[|【)DANMAKU_START(?:\]|】)([\s\S]*?)(?:(?:\[|【)\/?DANMAKU_END(?:\]|】)|(?:\[|【)OPTIONS_START|$)/i;
