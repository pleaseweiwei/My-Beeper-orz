...
window.renderCheckpointList = async function() {
    ...
    const chatId = _getActiveChatId();
    if (!chatId) {
        el.innerHTML = '<div style="color:#999;font-size: calc(12px * var(--font-scale));text-align:center;padding:20px;">请先打开一个聊天</div>';
        return;
    }
    const points = await listCheckpoints(chatId);
    if (points.length === 0) {
        el.innerHTML = '<div style="color:#999;font-size: calc(12px * var(--font-scale));text-align:center;padding:20px;">暂无存档，点击上方按钮创建</div>';
        return;
    }
    el.innerHTML = points.map(cp => `
        <div class="sk-summary-item" style="display:flex;justify-content:space-between;align-items:center;gap:10px;">
            <div style="flex:1;min-width:0;">
                <div style="font-size: calc(12px * var(--font-scale));font-weight:700;color:#333;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${cp.label}</div>
                <div style="font-size: calc(10px * var(--font-scale));color:#999;margin-top:2px;">好感度 ${Math.round((cp.friendSnapshot?.affection)||0)}%</div>
            </div>
            <div style="display:flex;gap:8px;flex-shrink:0;">
                <button class="sk-btn-secondary" style="padding:4px 10px;font-size: calc(11px * var(--font-scale));" onclick="loadCheckpoint('${cp.id}')"><i class="fas fa-undo"></i> 读档</button>
...
window.renderLinkMemoryUI = async function() {
    ...
    el.innerHTML = `
        ${allChats.length === 0
            ? '<div style="color:#ccc;font-size: calc(12px * var(--font-scale));text-align:center;padding:10px;">暂无其他聊天</div>'
            : allChats.map(c => `
                <label style="display:flex;align-items:center;gap:10px;padding:8px;border-radius:10px;background:#fafafa;margin-bottom:6px;cursor:pointer;">
                    ...
                    <span style="font-size: calc(13px * var(--font-scale));font-weight:600;color:#333;">${c.remark || c.realName || c.name || c.chatId} ${c.isGroup ? '<span style="color:#999;font-size: calc(11px * var(--font-scale));">(群聊)</span>' : ''}</span>
                </label>
            `).join('')}
