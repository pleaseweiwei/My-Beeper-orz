...
function _renderCharacterSelect() {
    ...
    if (!friends.length) {
        container.innerHTML = '<div style="color:#ccc;font-size:12px;padding:16px;text-align:center;">请先在 WeChat 中添加好友</div>';
        return;
    }
...
function _showGalDialog(speaker, text, isNarrator) {
    ...
    if (isNarrator) {
        ...
        nameEl.innerHTML = `<span style="opacity:0.55;font-size:10px;">✿</span>${speaker}`;
    }
    textEl.textContent = '';
    ...
    for (let i = 1; i <= GAL_MAX_SLOTS; i++) {
        ...
        if (save) {
            ...
            card.innerHTML = `
                ...
                <div class="gss-chars"><i class="fas fa-user-friends" style="opacity:.5;font-size:9px;"></i> ${save.charNames || '—'}</div>
                ${affSummary ? `<div class="gss-aff">${affSummary}</div>` : ''}
                <div class="gss-time"><i class="fas fa-clock" style="opacity:.4;font-size:9px;"></i> ${save.savedAt || ''}</div>
