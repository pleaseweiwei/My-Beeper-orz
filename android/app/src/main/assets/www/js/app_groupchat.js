...
window.openGroupChat = async function (groupId) {
    ...
    if (chatView) {
        ...
        if (titleEl) {
            ...
            titleEl.innerHTML = `${group.name}<small style="font-size: calc(9px * var(--font-scale)); color:#aaa; font-weight:400; margin-left:4px;">${memberCount}人</small>`;
        }
        // 修改设置图标的 onclick
    ...
    const renderGroupChatBatch = (isInitial = false) => {
        ...
        batch.forEach(msg => {
            if (msg.type === 'system') {
                ...
                sysDiv.innerHTML = `<span style="background:rgba(0,0,0,0.04); padding:4px 12px; border-radius:4px; font-size: calc(11px * var(--font-scale)); color:#999;">${msg.text}</span>`;
        ...
        if (window._currentChatRenderedCount >= total) {
             ...
             historyLabel.innerHTML = `<div style="text-align:center; margin: 10px 0;"><span style="background:rgba(0,0,0,0.04); padding:4px 12px; border-radius:12px; font-size: calc(10px * var(--font-scale)); color:#999; font-weight:500;">历史消息</span></div>`;
    ...
    if (history && history.length > 0) {
        ...
        sysDiv.innerHTML = `<span style="background:rgba(0,0,0,0.04); padding:4px 12px; border-radius:4px; font-size: calc(11px * var(--font-scale)); color:#999;">你邀请了 ${memberNames} 加入了群聊</span>`;
        chatMessages.appendChild(sysDiv);
...
function showAtMentionDropdown(group, query) {
    ...
    if (showAll) {
        ...
        allItem.innerHTML = `
            <div style="width:32px;height:32px;border-radius:50%;background:#eee;display:flex;align-items:center;justify-content:center;font-size: calc(14px * var(--font-scale));">👥</div>
            <span style="font-weight:600;font-size: calc(13px * var(--font-scale));">所有人</span>
        `;
    ...
    members.forEach(m => {
        ...
        item.innerHTML = `
            ...
            <span style="font-size: calc(13px * var(--font-scale));">${m.name}</span>
        `;
...
window.toggleAnonymousMode = function () {
    ...
    if (groupAnonymousMode) {
        ...
        if (!badge) {
            ...
            badge.style.cssText = 'text-align:center;font-size: calc(11px * var(--font-scale));color:#ff7e67;padding:4px;background:#fff5f3;border-radius:0;cursor:pointer;';
...
function showGroupAnnouncement(text, sendToChat) {
    ...
    if (sendToChat) {
        const chatMessages = document.getElementById('chatMessages');
        const div = document.createElement('div');
        div.style.cssText = 'margin:10px 15px; padding:10px 15px; background:#fffbe6; border-left:3px solid #ffd700; border-radius:8px; font-size: calc(12px * var(--font-scale)); color:#555;';
...
window.kickMember = async function (memberId, groupId) {
    ...
    if (chatMessages && currentChatId === groupId && currentChatType === 'group') {
        ...
        div.innerHTML = `<span style="background:rgba(0,0,0,0.04); padding:3px 10px; border-radius:4px; font-size: calc(11px * var(--font-scale)); color:#999;">${sysMsg}</span>`;
        chatMessages.appendChild(div);
    }
...
async function triggerAiGrabRedPacket(rpId, groupId) {
    ...
    setTimeout(() => {
        ...
        if (isLookingAtThisGroup) {
            ...
            const div = document.createElement('div');
            div.style.cssText = 'text-align:center; margin:8px 0;';
            div.innerHTML = `<span style="background:rgba(0,0,0,0.04); padding:3px 10px; border-radius:4px; font-size: calc(11px * var(--font-scale)); color:#999;">${sysText}</span>`;
            if (chatMessages) {
                chatMessages.appendChild(div);
                chatMessages.scrollTop = chatMessages.scrollHeight;
            }
...
window.grabRedPacket = async function (rpId) {
    ...
    const div = document.createElement('div');
    div.style.cssText = 'text-align:center; margin:8px 0;';
    div.innerHTML = `<span style="background:rgba(255,215,0,0.15); padding:3px 10px; border-radius:4px; font-size: calc(11px * var(--font-scale)); color:#b8860b;">${sysMsg}</span>`;
...
function renderRedPacketBubble(text) {
    ...
    return `
        <div class="group-red-packet-card" onclick="grabRedPacket('${rpId}')">
            <div class="rp-header">
                <i class="fas fa-envelope" style="color:#ffd700; font-size: calc(24px * var(--font-scale));"></i>
                <div class="rp-info">
...
function renderVoteBubble(tagText, voteId, groupId) {
    const group = groupsData[groupId];
    const voteData = group && group.votes && group.votes[voteId];
    if (!voteData) return `<div style="color:#999;font-size: calc(12px * var(--font-scale));">投票加载中...</div>`;
    ...
    return `
        <div class="group-vote-card" data-vote-id="${voteId}">
            ...
            <div class="gv-footer">
                ...
                <span style="color:#999; font-size: calc(11px * var(--font-scale));">${voteData.closed ? '已结束' : '进行中'}</span>
            </div>
        </div>
    `;
}
...
window.appendMessage = function (text, type, customAvatar, senderName, translation, msgId, timestamp) {
    ...
    if (typeof text === 'string' && text.startsWith('[RED_PACKET:')) {
        ...
        if (html) {
            ...
            let finalAvatar = 'icon.png';
            if (type === 'sent') {
                finalAvatar = (typeof AVATAR_USER !== 'undefined' ? AVATAR_USER : 'icon.png');
            } else {
                if (customAvatar) {
                    finalAvatar = customAvatar;
                } else if (typeof friendsData !== 'undefined' && typeof currentChatId !== 'undefined' && friendsData[currentChatId] && friendsData[currentChatId].avatar) {
                    finalAvatar = friendsData[currentChatId].avatar;
                } else if (typeof personasMeta !== 'undefined' && currentPersonaId && personasMeta[currentPersonaId] && personasMeta[currentPersonaId].avatar) {
                    finalAvatar = personasMeta[currentPersonaId].avatar;
                } else {
                    finalAvatar = `https://api.dicebear.com/7.x/avataaars/svg?seed=${senderName || 'AI'}`;
                }
            }
            img.src = finalAvatar;
            ...
            bubble.style.cssText = 'background:transparent!important;padding:0!important;box-shadow:none!important;';
...
function renderGsCssPresets () {
    ...
    Object.keys(presets).forEach(name => {
        ...
        chip.style.cssText = 'background:#f5f5f5;border-radius:8px;padding:4px 10px;font-size: calc(11px * var(--font-scale));cursor:pointer;display:flex;align-items:center;gap:6px;';
        chip.innerHTML = `<span>${name}</span><i class="fas fa-times" style="color:#ccc;font-size: calc(10px * var(--font-scale));"></i>`;
...
function loadGsLinkMemoryList (groupId) {
    ...
    Object.keys(groupsData).forEach(gId => {
        ...
        const item = document.createElement('div');
        item.className = 'checklist-item';
        item.innerHTML = `<input type="checkbox" value="${gId}" ${linkedIds.includes(gId) ? 'checked' : ''}><div style="width:28px;height:28px;border-radius:50%;background:#eee;display:flex;align-items:center;justify-content:center;font-size: calc(12px * var(--font-scale));flex-shrink:0;">👥</div><span class="checklist-name">${g.name}</span>`;
        item.onclick = (e) => { if (e.target.type !== 'checkbox') { const cb = item.querySelector('input'); if (cb) cb.checked = !cb.checked; } };
        container.appendChild(item);
    });
}
...
window.openGroupChatSearch = function () {
    ...
    if (resEl) resEl.innerHTML = `
        <div style="text-align:center;color:#ccc;font-size: calc(13px * var(--font-scale));padding:40px 0;">
...
window.performGroupChatSearch = async function () {
    ...
    if (!keyword) {
        resultsContainer.innerHTML = `<div style="text-align:center;color:#ccc;font-size: calc(13px * var(--font-scale));padding:40px 0;">输入关键词开始搜索</div>`;
        return;
    }
    ...
    resultsContainer.innerHTML = results.slice(-50).map(msg => {
        ...
        return `<div style="background:#fff;border-radius:12px;padding:12px 15px;margin-bottom:10px;border:1px solid #eee;">
            <div style="font-size: calc(11px * var(--font-scale));color:#aaa;margin-bottom:5px;">${sender}</div>
            <div style="font-size: calc(13px * var(--font-scale));color:#333;line-height:1.5;">${text}</div>
        </div>`;
    }).join('');
};
...
function renderGsCssPresets() {
    ...
    try {
        ...
        presets.forEach((preset) => {
            ...
            btn.style.cssText = 'background:#f5f5f7;border:none;border-radius:8px;padding:4px 10px;font-size: calc(11px * var(--font-scale));cursor:pointer;';
...
function renderGsLinkMemoryList(group) {
    ...
    allChats.forEach(chat => {
        ...
        item.style.cssText = 'display:flex;align-items:center;gap:8px;padding:6px 8px;cursor:pointer;font-size: calc(12px * var(--font-scale));background:#fafafa;border-radius:10px;margin-bottom:6px;';
        ...
        span.style.cssText = 'font-size: calc(13px * var(--font-scale));font-weight:600;color:#333;';
    ...
    if (container.children.length === 0) {
        container.innerHTML = '<div style="color:#ccc;font-size: calc(12px * var(--font-scale));text-align:center;padding:15px;">暂无可选聊天</div>';
    } else {
        const depthContainer = document.createElement('div');
        depthContainer.style.cssText = 'margin-top:10px;';
        depthContainer.innerHTML = `
            <label style="font-size: calc(12px * var(--font-scale));font-weight:600;color:#555;">互通条数 (最近N条)</label>
...
window.openGroupChatSearch = function () {
    ...
    if (results) results.innerHTML = '<div style="text-align:center;color:#ccc;font-size: calc(13px * var(--font-scale));padding:40px 0;"><i class="fas fa-search" style="font-size: calc(32px * var(--font-scale));margin-bottom:10px;display:block;opacity:0.3;"></i>输入关键词开始搜索</div>';
    page.classList.add('show');
};
...
window.performGroupChatSearch = async function () {
    ...
    if (filtered.length === 0) {
        results.innerHTML = '<div style="text-align:center;color:#bbb;padding:30px;font-size: calc(13px * var(--font-scale));">没有找到相关消息</div>';
        return;
    }
    ...
    results.innerHTML = filtered.map(m => {
        ...
        return `<div style="padding:12px;background:#fff;border-radius:12px;margin-bottom:8px;box-shadow:0 1px 4px rgba(0,0,0,0.05);">
            <div style="font-size: calc(11px * var(--font-scale));color:#999;margin-bottom:4px;">${sender}</div>
            <div style="font-size: calc(13px * var(--font-scale));color:#333;">${text}</div>
        </div>`;
    }).join('');
};
...
function _appendGroupOfflineSystemEntry(text) {
    ...
    div.innerHTML = `<div class="oe-text" style="text-align:center;color:#aaa;font-size: calc(12px * var(--font-scale));font-style:italic;padding:6px 0;">${
    ...
    if (role === 'ai') {
        ...
        div.innerHTML = `
            <div style="display:flex;align-items:center;gap:7px;margin-bottom:5px;">
                ${avatarHtml}
                <div class="oe-name" style="font-size: calc(11px * var(--font-scale));font-weight:700;letter-spacing:0.5px;">${name}</div>
            </div>
...
window.refreshGroupMindCardUI = function(groupId, useTyping = false) {
    ...
    if (members.length === 0) {
        listContainer.innerHTML = '<div style="text-align:center; color:#999; font-size: calc(12px * var(--font-scale)); padding:20px;">群里暂时没有人的心声...</div>';
        return;
    }
            ...
            <div class="group-mind-item" style="padding: 16px; border-bottom: 1px dashed #f0f0f0; display: flex; flex-direction: column; gap: 12px;">
                <div style="display: flex; align-items: center;">
                    ...
                    <div style="margin-left: 12px; flex: 1;">
                        <div style="font-size: calc(14px * var(--font-scale)); font-weight: 800; color: #111; letter-spacing: 0.5px; margin-bottom: 4px;">${name}</div>
                    </div>
                </div>
                <div style="background: #f9f9f9; border-radius: 12px; padding: 12px 14px; position: relative;">
                    <i class="fas fa-quote-left" style="color: #ddd; font-size: calc(12px * var(--font-scale)); position: absolute; top: -6px; left: 12px; background: #fff; padding: 0 4px;"></i>
                    <div class="group-mind-text" id="gm-text-${memberId}" style="font-size: calc(13px * var(--font-scale)); color: #444; line-height: 1.6; font-style: normal;">${murmur}</div>
                </div>
            </div>
        `;
