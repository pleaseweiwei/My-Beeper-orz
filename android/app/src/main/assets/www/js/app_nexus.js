...
const NexusApp = (() => {
        ...
        '<div class="nexus-forge-content" id="nexus-forge-content">',
          ...
          '<div class="nexus-forge-step-view" id="forge-step-3" style="display:none">',
            '<div id="forge-loading" class="nexus-forge-loading">',
              ...
              '<div style="font-size: calc(11px * var(--font-scale));color:rgba(255,255,255,0.4);letter-spacing:2px">正在孕育生命</div>',
  ...
  function openForgePanel() {
    ...
    if (wbList) {
      ...
      if (!books.length) {
        wbList.innerHTML = '<div style="color:rgba(255,255,255,0.3);font-size: calc(12px * var(--font-scale));padding:8px 0">暂无世界书，请先在世界书应用中创建</div>';
      } else {
    ...
    if (pList) {
      ...
      if (personas.length) {
        html += '<div style="font-size: calc(9px * var(--font-scale));color:rgba(255,255,255,0.3);letter-spacing:1.5px;font-weight:700;margin-bottom:8px;text-transform:uppercase">主人设</div>';
        ...
        html += '<div style="color:rgba(255,255,255,0.3);font-size: calc(12px * var(--font-scale));padding:4px 0 8px">暂无主人设</div>';
      }

      // Sub-section: AI Friends
      if (aiFriends.length) {
        html += '<div style="font-size: calc(9px * var(--font-scale));color:rgba(255,255,255,0.3);letter-spacing:1.5px;font-weight:700;margin:14px 0 8px;text-transform:uppercase">AI 好友</div>';
      ...
      if (!personas.length && !aiFriends.length) {
        html = '<div style="color:rgba(255,255,255,0.3);font-size: calc(12px * var(--font-scale));padding:8px 0">暂无好友，请先在微信中添加好友</div>';
      }
  ...
  function createMeNode(center) {
    ...
    el.innerHTML = '<div class="nexus-node-inner" style="background:#111111;">' +
      (src ? '<img src="' + src + '" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">' :
             '<span style="font-size: calc(20px * var(--font-scale));font-weight:800;color:#FFF;">' + name.charAt(0) + '</span>') +
  ...
  function createNodeElement(node) {
    ...
    const content = node.avatar
      ...
      : '<span style="font-size: calc(17px * var(--font-scale));font-weight:800;color:#FFF;">' + (node.name || '?').charAt(0) + '</span>';
    el.innerHTML =
  ...
  function openProfileSheet(nodeId) {
    ...
    if (avatarEl) {
      avatarEl.innerHTML = node.avatar
        ...
        : '<div style="width:100%;height:100%;border-radius:50%;background:linear-gradient(135deg,' + c0 + ',' + c1 + ');display:flex;align-items:center;justify-content:center;font-size: calc(24px * var(--font-scale));font-weight:800;color:#fff">' + (node.name || '?').charAt(0) + '</div>';
    }
  ...
  function showToast(msg) {
    ...
    t.style.cssText = 'position:fixed;bottom:90px;left:50%;transform:translateX(-50%);' +
      ...
      'padding:10px 20px;border-radius:20px;font-size: calc(12px * var(--font-scale));font-weight:600;' +
