...
function renderPetMomentsList() {
    ...
    if (petData.moments.length === 0) {
        list.innerHTML = '<div style="text-align:center; padding:40px; color:#999; font-size: calc(12px * var(--font-scale));">空空如也... 点击右上角魔法棒生成第一条动态！</div>';
        return;
    }
...
window.openPetAlbum = function() {
    ...
    if (petData.polaroids.length === 0) {
        grid.innerHTML = '<div style="grid-column:span 2; text-align:center; padding:20px; color:#999; font-size: calc(12px * var(--font-scale));">暂无回忆...快去旅行吧！</div>';
    } else {
...
window.petFeed = function() {
    ...
    if ((petData.todayFeedCount || 0) < 3) {
        ...
        executeFeed();
        showCustomDialog('🍖', `啊呜啊呜！吃饱啦！<br>成长值 +5<br><span style="font-size: calc(10px * var(--font-scale));color:#999;">今日免费喂食剩余: ${3 - petData.todayFeedCount}次</span>`);
    } else {
...
window.petPlay = function() {
    ...
    if ((petData.todayPlayCount || 0) < 3) {
        ...
        savePetData();
        updatePetStatsUI();
        showCustomDialog('🎾', `追着跑了好久！开心！<br>成长值 +5<br><span style="font-size: calc(10px * var(--font-scale));color:#999;">今日免费玩耍剩余: ${3 - petData.todayPlayCount}次</span>`);
    } else {
    ...
    items.forEach(item => {
        ...
        div.innerHTML = `
            <div style="display:flex; gap:10px; align-items:center;">
                <div style="font-size: calc(24px * var(--font-scale));">${item.icon}</div>
                <div>
                    <div style="font-weight:700; font-size: calc(13px * var(--font-scale));">${item.name}</div>
                    <div style="font-size: calc(10px * var(--font-scale)); color:#999;">${item.desc}</div>
                </div>
            </div>
            <button class="shop-buy-btn" onclick="buyItem('${type}', '${item.id}')" style="background:#333; color:#fff; border:none; padding:5px 12px; border-radius:15px; font-size: calc(11px * var(--font-scale));">
                ${item.price} ✨
            </button>
        `;
        list.appendChild(div);
