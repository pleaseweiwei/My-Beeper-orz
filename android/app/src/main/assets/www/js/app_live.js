...
function renderLiveUI() {
    ...
    if (worksContainer) {
        ...
        liveData.posts.forEach(p => {
            ...
            if (p.mediaType === 'video') {
                div.innerHTML = `<video src="${p.media}" style="width:100%;height:100%;object-fit:cover;"></video><i class="fas fa-video" style="position:absolute;top:5px;right:5px;color:#fff;font-size: calc(10px * var(--font-scale));"></i>`;
                div.style.position = 'relative';
            } else if (p.media) {
                div.innerHTML = `<img src="${p.media}" style="width:100%;height:100%;object-fit:cover;">`;
            } else {
                div.innerHTML = `<div style="width:100%;height:100%;background:#eee;display:flex;align-items:center;justify-content:center;font-size: calc(10px * var(--font-scale));color:#888;text-align:center;">${p.imgDesc || 'TEXT'}</div>`;
            }
    ...
    if (discContainer) {
        ...
        liveData.discover.forEach(p => {
            ...
            let mediaHtml = '';
            if (p.mediaType === 'video') {
                mediaHtml = `<video src="${p.media}" style="width:100%;aspect-ratio:3/4;object-fit:cover;"></video><i class="fas fa-play-circle" style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);color:rgba(255,255,255,0.8);font-size: calc(36px * var(--font-scale));pointer-events:none;"></i>`;
            } else if (p.media) {
                mediaHtml = `<img src="${p.media}" style="width:100%;aspect-ratio:3/4;object-fit:cover;">`;
            } else {
                mediaHtml = `<div style="width:100%;aspect-ratio:3/4;background:#eee;display:flex;align-items:center;justify-content:center;padding:20px;font-size: calc(12px * var(--font-scale));color:#555;text-align:center;">${p.imgDesc || 'TEXT POST'}</div>`;
            }
...
window.openLiveQuickIdentityModal = function() {
    ...
    Object.values(personasMeta).forEach(p => {
        ...
        div.innerHTML = `
            ...
            <div style="font-weight:700; font-size: calc(14px * var(--font-scale));">${p.name}</div>
...
window.handleLiveMediaUpload = function(input) {
    if (input.files && input.files[0]) {
        ...
        reader.onload = async function(e) {
            ...
            if (isVideo) {
                preview.innerHTML = `<i class="fas fa-play-circle" style="color:#fff; font-size: calc(24px * var(--font-scale)); position:absolute; z-index:2;"></i><video src="${resData}" style="width:100%;height:100%;object-fit:cover;"></video>`;
...
window.publishLivePost = async function() {
    ...
    document.getElementById('live-post-img-desc').value = '';
    preview.innerHTML = '<i class="fas fa-camera" style="color:#aaa; font-size: calc(20px * var(--font-scale));"></i>';
    preview.dataset.src = '';
...
window.openTikTokView = function(postId) {
    ...
    if (targetPost.mediaType === 'video') {
        ...
        mediaContainer.innerHTML = `<div style="width:100%; height:100%; background:linear-gradient(135deg, #2b2b2b, #111); display:flex; align-items:center; justify-content:center; padding:40px; color:#fff; font-size: calc(18px * var(--font-scale)); font-family:'Songti SC', serif; text-align:center; line-height:1.6;">${targetPost.imgDesc || 'TEXT POST'}</div>`;
    }
        ...
        targetPost.comments.forEach(c => {
            list.innerHTML += `
                <div style="display:flex; gap:12px;">
                    ...
                    <div style="flex:1;">
                        <div style="font-size: calc(12px * var(--font-scale)); font-weight:700; color:${c.isAI ? '#ff2c55' : '#888'}; margin-bottom:4px;">${c.name}</div>
                        <div style="font-size: calc(14px * var(--font-scale)); color:#333; line-height:1.4;">${c.text}</div>
                    </div>
                </div>
            `;
        });
    } else {
        list.innerHTML = `<div style="text-align:center; color:#ccc; font-size: calc(12px * var(--font-scale)); margin-top:40px;">留下第一条评论吧</div>`;
    }
