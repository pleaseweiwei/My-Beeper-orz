...
window.ls2Disconnect = async function () {
  ...
  ls2Confirm(
    '解除灵魂绑定',
    "确定要解除绑定并清空该空间的数据吗？<br><br><span style='color:#ff4d4f; font-size: calc(11px * var(--font-scale));'>此操作无法恢复</span>",
    async () => {
...
function renderLs2Journal() {
  ...
  stickers.forEach(s => {
    ...
    if (s.type === 'note') {
      ...
      innerStyle += `font-family:${fontF}; font-size: calc(18px * var(--font-scale)); padding:15px;`;
...
function renderLs2Fridge() {
  ...
  (ls2Data.fridgeOut || []).forEach(n => {
    ...
    if (n.type === 'note') {
      ...
      el.innerHTML = `
        <div style="font-size: calc(10px * var(--font-scale)); color:#aaa; margin-bottom:6px;">${who}</div>
...
{"diet":"...", "calories": 1200}
  ...
  try {
    const data = JSON.parse(res.replace(/```json/gi,'').replace(/```/g,'').trim());
    box.innerHTML = `<b>${ai.realName || 'TA'} 的饮食查岗:</b><br>${String(data.diet||'').replace(/</g,'&lt;').replace(/>/g,'&gt;')}<br><span style="color:#aaa;font-size: calc(10px * var(--font-scale));">预估热量: ${data.calories} kcal</span>`;
  ...
  arr.slice(1).forEach(i => {
    history.innerHTML += `
      <div class="qa-history-item">
        <div class="qa-hi-q" style="font-size: calc(12px * var(--font-scale));font-weight:bold;color:#111;margin-bottom:4px;">${i.q}</div>
        <div class="qa-hi-a" style="font-size: calc(11px * var(--font-scale));color:#666;">我: ${i.myA}<br>TA: ${i.aiA}</div>
      </div>
    `;
  });
}
...
function renderCookIngredients() {
    ...
    (ls2Data.fridgeIn || []).forEach(item => {
        ...
        html += `<div style="padding: 8px 12px; background: #f0f0f0; border-radius: 20px; cursor: pointer; display: flex; align-items: center; gap: 5px; font-size: calc(13px * var(--font-scale));" onclick="selectCookIngredient(${item.id})">
            ${item.emoji} ${item.name}
        </div>`;
    });
    
    if (!html) html = '<div style="color:#aaa; font-size: calc(12px * var(--font-scale));">没有更多可用食材了</div>';
...
function renderCookSelected() {
    ...
    currentCookSelectedIds.forEach(id => {
        const item = ls2Data.fridgeIn.find(x => x.id === id);
        if (item) {
            html += `<div style="padding: 8px 12px; background: #fff4e6; border: 1px solid #ffd8a8; border-radius: 20px; cursor: pointer; display: flex; align-items: center; gap: 5px; font-size: calc(13px * var(--font-scale));" onclick="deselectCookIngredient(${item.id})">
                ${item.emoji} ${item.name} <i class="fas fa-times" style="font-size: calc(10px * var(--font-scale)); color:#ff7e67; margin-left:4px;"></i>
            </div>`;
        }
    });
    
    if (!html) html = '<div style="color:#aaa; font-size: calc(12px * var(--font-scale)); align-self: center;">点击上方食材添加到这里</div>';
    container.innerHTML = html;
}
