...
function bbOpenBeansWallet() {
    ...
    let txHtml = (d.transactions || []).slice(0, 50).map(t => {
        ...
        return `<div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid #f5f5f5;">
            <div><div style="font-size: calc(13px * var(--font-scale));font-weight:600;color:#333;">${t.reason}</div><div style="font-size: calc(10px * var(--font-scale));color:#bbb;margin-top:2px;">${dt}</div></div>
            <div style="font-size: calc(16px * var(--font-scale));font-weight:800;color:${color};">${sign}${t.amount}</div>
        </div>`;
    }).join('') || '<div style="text-align:center;color:#bbb;padding:30px;font-size: calc(12px * var(--font-scale));">暂无记录</div>';
    ...
    modal.style.cssText = 'z-index:6500;';
        ...
        <div class="modal-box" style="width: 90%; max-width: 320px; max-height: 80vh; display:flex; flex-direction:column; border-radius:24px; background:#fff; box-sizing: border-box;">
            <div class="modal-header" style="border-bottom:none; padding: 20px 20px 5px; box-sizing: border-box;">
                <span style="font-size: calc(15px * var(--font-scale)); font-weight:800;">豆币钱包</span>
            ...
            <div style="padding:25px 15px; background:linear-gradient(135deg,#f9f9f9,#f0f0f0); margin:0 20px 15px; border-radius:20px; color:#555; text-align:center; border:1px solid #eee; box-shadow:inset 0 2px 10px rgba(255,255,255,1);">
                <div style="font-size: calc(10px * var(--font-scale)); color:#999; font-weight:700; font-family:'Montserrat'; letter-spacing:1px;">BALANCE (BEANS)</div>
                <div style="font-size: calc(36px * var(--font-scale)); font-weight:300; font-family:'Montserrat',sans-serif; margin:8px 0; color:#333;">${d.beanCoins || 0}</div>
                ${isIdol ? `<div style="font-size: calc(11px * var(--font-scale)); color:#ff7e67; font-weight:600;">收益账户: ${d.idolRevenue || 0} 豆币</div>` : ''}
            </div>
            <div style="display:flex; gap:10px; padding:0 20px 15px; box-sizing: border-box;">
                ${!isIdol ? `<button onclick="bbRechargeBeansPrompt('${aiId}',this)" style="flex:1; padding:12px 0; border-radius:12px; background:#fff; color:#555; border:1px solid #eee; font-size: calc(12px * var(--font-scale)); font-weight:600; cursor:pointer;">充值豆币</button>` : ''}
                ${isIdol ? `<button onclick="bbWithdrawRevenue('${aiId}',this)" style="flex:1; padding:12px 0; border-radius:12px; background:#f5f5f5; color:#555; border:1px solid #eee; font-size: calc(12px * var(--font-scale)); font-weight:600; cursor:pointer;">提现到钱包</button>` : ''}
            </div>
            <div style="flex:1; overflow-y:auto; padding:0 20px 20px; box-sizing: border-box;">
                <div style="font-size: calc(10px * var(--font-scale)); font-weight:700; color:#aaa; letter-spacing:1px; margin-bottom:8px;">TRANSACTIONS</div>
                ${txHtml}
            </div>
        </div>`;
...
function bbRenderLanding() {
    ...
    if (lastFanAi && lastFanMe) {
        ...
        <div style="display:flex; align-items:center; gap:12px; background:rgba(255,255,255,0.9); padding:16px; border-radius:20px; border:1px solid #f0f0f0; margin-bottom:12px; cursor:pointer; box-shadow:0 8px 25px rgba(0,0,0,0.02); transition:0.2s;" onclick="bbResumeFanMode('${lastFanAi}', '${lastFanMe}')">
            ...
            <div style="flex:1; text-align:left;">
                <div style="font-size: calc(9px * var(--font-scale)); color:#aaa; font-weight:700; font-family:'Montserrat'; letter-spacing:1px; margin-bottom:4px;">FAN MODE</div>
                <div style="font-size: calc(14px * var(--font-scale)); font-weight:800; color:#444;">${aiName}</div>
            </div>
            <i class="fas fa-chevron-right" style="color:#ddd; font-size: calc(14px * var(--font-scale));"></i>
        </div>
        `;
    }
    ...
    if (lastIdolMe) {
        ...
        <div style="display:flex; align-items:center; gap:12px; background:#f5f5f5; color:#555; padding:16px; border-radius:20px; margin-bottom:20px; border:1px solid #eaeaea; cursor:pointer; box-shadow:inset 0 2px 5px rgba(255,255,255,1); transition:0.2s;" onclick="bbResumeIdolMode('${lastIdolMe}')">
            ...
            <div style="flex:1; text-align:left;">
                <div style="font-size: calc(9px * var(--font-scale)); color:#888; font-weight:700; font-family:'Montserrat'; letter-spacing:1px; margin-bottom:4px;">CREATOR STUDIO</div>
                <div style="font-size: calc(14px * var(--font-scale)); font-weight:800; color:#333;">${pName}</div>
            </div>
            <i class="fas fa-chevron-right" style="color:#ccc; font-size: calc(14px * var(--font-scale));"></i>
        </div>
        `;
    }
    ...
    if (container) {
        container.innerHTML = html ? `<div style="font-size: calc(10px * var(--font-scale)); color:#aaa; margin-bottom:10px; font-weight:800; letter-spacing:1px;">RECENT CONNECTION</div>${html}` : '';
    }
}
...
window.bbEnterFanModeFlow = async function() {
    ...
    <div class="modal-box" style="width:340px;border-radius:24px;padding:0;overflow:hidden;background:#fff;border:1px solid #f0f0f0;">
        <div class="modal-header" style="padding:24px 24px 10px;border-bottom:none;background:#fff;">
            <span style="font-size: calc(18px * var(--font-scale));font-weight:800;color:#111;font-family:'Montserrat', sans-serif;">FAN MODE.</span>
            <i class="fas fa-times" onclick="this.closest('.modal-overlay').remove()" style="cursor:pointer;color:#bbb;font-size: calc(18px * var(--font-scale));"></i>
        </div>
        <div style="padding:0 24px 24px;display:flex;flex-direction:column;gap:20px;background:#fff;">
            <div style="font-size: calc(11px * var(--font-scale));color:#888;line-height:1.6;background:#fafafa;padding:12px;border-radius:12px;border:1px solid #eee;">
                选择你的人设和想追的AI爱豆。进入主界面后，可在右上角 ⚙️ 补充专属设定。
            </div>
            <div>
                <div style="font-size: calc(11px * var(--font-scale));font-weight:700;color:#666;margin-bottom:8px;">我的人设 <span style="color:#aaa;font-weight:400;">Persona</span></div>
                <select id="bb-flow-my-persona" style="width:100%;border:1px solid #eee;border-radius:12px;padding:12px;font-size: calc(13px * var(--font-scale));outline:none;background:#fafafa;color:#555;" onchange="bbUpdateAiFriendsList(this.value)">
                    ${personaOptions}
                </select>
            </div>
            <div>
                <div style="font-size: calc(11px * var(--font-scale));font-weight:700;color:#666;margin-bottom:8px;">AI 爱豆 <span style="color:#aaa;font-weight:400;">Idol</span></div>
                <select id="bb-flow-ai-friend" style="width:100%;border:1px solid #eee;border-radius:12px;padding:12px;font-size: calc(13px * var(--font-scale));outline:none;background:#fafafa;color:#555;">
                    <!-- 动态加载好友列表 -->
                </select>
            </div>
            <button onclick="bbConfirmFanModeEntry(this)" style="width:100%;padding:16px;background:#f5f5f5;color:#666;border:1px solid #eee;border-radius:16px;font-weight:800;font-size: calc(13px * var(--font-scale));cursor:pointer;margin-top:10px;letter-spacing:1px; transition:0.2s;">JOIN ✦</button>
        </div>
    </div>`;
...
window.bbEnterIdolModeFlow = function() {
    ...
    <div class="modal-box" style="width:340px;border-radius:24px;padding:0;overflow:hidden;background:#fff;border:1px solid #f0f0f0;">
        <div class="modal-header" style="padding:24px 24px 10px;border-bottom:none;background:#fff;">
            <span style="font-size: calc(18px * var(--font-scale));font-weight:800;color:#111;font-family:'Montserrat', sans-serif;">IDOL MODE.</span>
            <i class="fas fa-times" onclick="this.closest('.modal-overlay').remove()" style="cursor:pointer;color:#bbb;font-size: calc(18px * var(--font-scale));"></i>
        </div>
        <div style="padding:0 24px 24px;display:flex;flex-direction:column;gap:20px;background:#fff;">
            <div style="font-size: calc(11px * var(--font-scale));color:#888;line-height:1.6;background:#fafafa;padding:12px;border-radius:12px;border:1px solid #eee;">
            ...
            <div>
                <div style="font-size: calc(11px * var(--font-scale));font-weight:700;color:#666;margin-bottom:8px;">我的人设 <span style="color:#aaa;font-weight:400;">Persona</span></div>
                <select id="bb-flow-idol-persona" style="width:100%;border:1px solid #eee;border-radius:12px;padding:12px;font-size: calc(13px * var(--font-scale));outline:none;background:#fafafa;color:#555;">
                    ${personaOptions}
                </select>
            </div>
            <button onclick="bbConfirmIdolModeEntry(this)" style="width:100%;padding:16px;background:#f5f5f5;color:#666;border:1px solid #eee;border-radius:16px;font-weight:800;font-size: calc(13px * var(--font-scale));cursor:pointer;margin-top:10px;letter-spacing:1px; transition:0.2s;">START ✦</button>
        </div>
    </div>`;
...
window.bbOpenFanModeSettings = function(aiId) {
    ...
    <div class="modal-box" style="width:340px;max-height:90vh;overflow-y:auto;border-radius:24px;padding:0;background:#fff;">
        <div class="modal-header" style="padding:24px 24px 10px;border-bottom:none;">
            <span style="font-size: calc(16px * var(--font-scale));font-weight:800;color:#111;font-family:'Montserrat', sans-serif;">SETTING.</span>
            <i class="fas fa-times" onclick="this.closest('.modal-overlay').remove()" style="cursor:pointer;color:#aaa;font-size: calc(18px * var(--font-scale));"></i>
        </div>
        <div style="padding:0 24px 24px;display:flex;flex-direction:column;gap:16px;">
            <div style="background:#f9f9f9;border-radius:12px;padding:16px;border:1px solid #eee;">
                <div style="font-size: calc(11px * var(--font-scale));font-weight:700;color:#111;letter-spacing:1px;margin-bottom:8px;"><i class="fas fa-link"></i> 基础人设（只读）</div>
                <div style="font-size: calc(12px * var(--font-scale));color:#666;line-height:1.5;max-height:60px;overflow-y:auto;padding-right:4px;">${wcPersona || '（未设置人设）'}</div>
                ${wcWorldbook ? `<div style="font-size: calc(10px * var(--font-scale));color:#aaa;margin-top:8px;border-top:1px dashed #ddd;padding-top:8px;">${wcWorldbook.slice(0,50)}${wcWorldbook.length>50?'...':''}</div>` : ''}
            </div>
            
            <div>
                <div style="font-size: calc(11px * var(--font-scale));font-weight:700;color:#333;margin-bottom:6px;letter-spacing:1px;">Bubble 专属宇宙背景</div>
                <textarea id="fms-worldbook" rows="2" style="width:100%;border:1px solid #eee;border-radius:12px;padding:12px;font-size: calc(12px * var(--font-scale));resize:none;outline:none;background:#fafafa;" placeholder="叠加在基础人设之上的背景设定...">${s.worldbook||''}</textarea>
            </div>
            
            <div style="display:flex;gap:12px;">
                <div style="flex:1;">
                    <div style="font-size: calc(11px * var(--font-scale));font-weight:700;color:#333;margin-bottom:6px;letter-spacing:1px;">团队担当</div>
                    <input type="text" id="fms-team" value="${(s.idolInfo&&s.idolInfo.team)||''}" style="width:100%;border:1px solid #eee;border-radius:12px;padding:10px;font-size: calc(12px * var(--font-scale));outline:none;background:#fafafa;" placeholder="如：队长/主唱">
                </div>
                <div style="flex:1;">
                    <div style="font-size: calc(11px * var(--font-scale));font-weight:700;color:#333;margin-bottom:6px;letter-spacing:1px;">特长</div>
                    <input type="text" id="fms-skills" value="${(s.idolInfo&&s.idolInfo.skills)||''}" style="width:100%;border:1px solid #eee;border-radius:12px;padding:10px;font-size: calc(12px * var(--font-scale));outline:none;background:#fafafa;" placeholder="如：舞蹈/作曲">
                </div>
            </div>
            
            <div style="display:flex;gap:12px;">
                <div style="flex:1;">
                    <div style="font-size: calc(11px * var(--font-scale));font-weight:700;color:#333;margin-bottom:6px;letter-spacing:1px;">经纪公司</div>
                    <input type="text" id="fms-company" value="${(s.idolInfo&&s.idolInfo.company)||''}" style="width:100%;border:1px solid #eee;border-radius:12px;padding:10px;font-size: calc(12px * var(--font-scale));outline:none;background:#fafafa;" placeholder="如：SM">
                </div>
                <div style="flex:1;">
                    <div style="font-size: calc(11px * var(--font-scale));font-weight:700;color:#333;margin-bottom:6px;letter-spacing:1px;">队友</div>
                    <input type="text" id="fms-teammates" value="${(s.idolInfo&&s.idolInfo.teammates)||''}" style="width:100%;border:1px solid #eee;border-radius:12px;padding:10px;font-size: calc(12px * var(--font-scale));outline:none;background:#fafafa;" placeholder="逗号分隔">
                </div>
            </div>
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;padding:16px;background:#fafafa;border-radius:12px;border:1px solid #eee;">
                <div>
                    <div style="font-size: calc(12px * var(--font-scale));font-weight:700;color:#111;">连通主控记忆</div>
                    <div style="font-size: calc(10px * var(--font-scale));color:#888;margin-top:2px;">互动将写入微信主聊天</div>
                </div>
            ...
            <div>
                <div style="font-size: calc(11px * var(--font-scale));font-weight:700;color:#333;margin-bottom:6px;letter-spacing:1px;">自动发帖间隔 (秒)</div>
                <input type="number" id="fms-auto-post" value="${s.autoPostInterval||300}" style="width:100%;border:1px solid #eee;border-radius:12px;padding:10px;font-size: calc(13px * var(--font-scale));outline:none;background:#fafafa;">
            </div>
            
            <button onclick="bbSaveFanModeSettings('${aiId}')" style="width:100%;padding:16px;background:#111;color:#fff;border:none;border-radius:16px;font-weight:800;font-size: calc(13px * var(--font-scale));cursor:pointer;letter-spacing:1px;margin-top:8px;">SAVE SETTING</button>
        </div>
    </div>`;
    document.body.appendChild(modal);
}
...
window.bbOpenIdolModeSettings = function(aiId) {
    ...
    <div class="modal-box" style="width:340px;max-height:90vh;overflow-y:auto;border-radius:24px;padding:0;background:#fff;">
        <div class="modal-header" style="padding:24px 24px 10px;border-bottom:none;background:#fff;">
            <span style="font-size: calc(16px * var(--font-scale));font-weight:800;color:#111;font-family:'Montserrat', sans-serif;">MY IDOL PROFILE.</span>
            <i class="fas fa-times" onclick="this.closest('.modal-overlay').remove()" style="cursor:pointer;color:#aaa;font-size: calc(18px * var(--font-scale));"></i>
        </div>
        <div style="padding:0 24px 24px;display:flex;flex-direction:column;gap:16px;">
            <div style="background:#f9f9f9;border-radius:12px;padding:16px;border:1px solid #eee;">
                <div style="font-size: calc(11px * var(--font-scale));font-weight:700;color:#111;letter-spacing:1px;margin-bottom:8px;"><i class="fas fa-link"></i> ${myName} 主线设定（只读）</div>
                <div style="font-size: calc(12px * var(--font-scale));color:#666;line-height:1.5;max-height:60px;overflow-y:auto;padding-right:4px;">${myWcPersona || '（未设置人设）'}</div>
                ${myWcWorldbook ? `<div style="font-size: calc(10px * var(--font-scale));color:#aaa;margin-top:8px;border-top:1px dashed #ddd;padding-top:8px;">${myWcWorldbook.slice(0,50)}${myWcWorldbook.length>50?'...':''}</div>` : ''}
            </div>
            
            <div>
                <div style="font-size: calc(11px * var(--font-scale));font-weight:700;color:#333;margin-bottom:6px;letter-spacing:1px;">Bubble 专属宇宙背景</div>
                <textarea id="ims-worldbook" rows="2" style="width:100%;border:1px solid #eee;border-radius:12px;padding:12px;font-size: calc(12px * var(--font-scale));resize:none;outline:none;background:#fafafa;" placeholder="叠加在基础人设之上的背景设定...">${s.worldbook||''}</textarea>
            </div>
            
            <div style="display:flex;gap:12px;">
                <div style="flex:1;">
                    <div style="font-size: calc(11px * var(--font-scale));font-weight:700;color:#333;margin-bottom:6px;letter-spacing:1px;">团队担当</div>
                    <input type="text" id="ims-team" value="${(s.myIdolInfo&&s.myIdolInfo.team)||''}" style="width:100%;border:1px solid #eee;border-radius:12px;padding:10px;font-size: calc(12px * var(--font-scale));outline:none;background:#fafafa;" placeholder="如：队长/主唱">
                </div>
                <div style="flex:1;">
                    <div style="font-size: calc(11px * var(--font-scale));font-weight:700;color:#333;margin-bottom:6px;letter-spacing:1px;">特长</div>
                    <input type="text" id="ims-skills" value="${(s.myIdolInfo&&s.myIdolInfo.skills)||''}" style="width:100%;border:1px solid #eee;border-radius:12px;padding:10px;font-size: calc(12px * var(--font-scale));outline:none;background:#fafafa;" placeholder="如：舞蹈/作曲">
                </div>
            </div>
            ...
            <div style="display:flex;gap:12px;">
                <div style="flex:1;">
                    <div style="font-size: calc(11px * var(--font-scale));font-weight:700;color:#333;margin-bottom:6px;letter-spacing:1px;">经纪公司</div>
                    <input type="text" id="ims-company" value="${(s.myIdolInfo&&s.myIdolInfo.company)||''}" style="width:100%;border:1px solid #eee;border-radius:12px;padding:10px;font-size: calc(12px * var(--font-scale));outline:none;background:#fafafa;" placeholder="如：SM">
                </div>
                <div style="flex:1;">
                    <div style="font-size: calc(11px * var(--font-scale));font-weight:700;color:#333;margin-bottom:6px;letter-spacing:1px;">队友</div>
                    <input type="text" id="ims-teammates" value="${(s.myIdolInfo&&s.myIdolInfo.teammates)||''}" style="width:100%;border:1px solid #eee;border-radius:12px;padding:10px;font-size: calc(12px * var(--font-scale));outline:none;background:#fafafa;" placeholder="逗号分隔">
                </div>
            </div>
            ...
            <div>
                <div style="font-size: calc(11px * var(--font-scale));font-weight:700;color:#333;margin-bottom:6px;letter-spacing:1px;">危机触发间隔 (秒)</div>
                <input type="number" id="ims-crisis" value="${s.crisisInterval||3600}" style="width:100%;border:1px solid #eee;border-radius:12px;padding:10px;font-size: calc(13px * var(--font-scale));outline:none;background:#fafafa;">
            </div>
            
            <button onclick="bbSaveIdolModeSettings('${aiId}')" style="width:100%;padding:16px;background:#111;color:#fff;border:none;border-radius:16px;font-weight:800;font-size: calc(13px * var(--font-scale));cursor:pointer;letter-spacing:1px;margin-top:8px;">SAVE SETTING</button>
        </div>
    </div>`;
    document.body.appendChild(modal);
}
...
function bbInitFanMode(aiId) {
    ...
    if (daysEl) {
        if (s.subscriptionDays > 0) {
            ...
            daysEl.innerHTML = `<button onclick="bbSubscribeAction('${aiId}')" style="background:#ff7e67; color:#fff; border:none; border-radius:10px; padding:3px 10px; font-size: calc(10px * var(--font-scale)); cursor:pointer; box-shadow:0 2px 8px rgba(255,126,103,0.3);">点击订阅</button>`;
        }
    }
...
function bbRenderPublicPosts(aiId) {
    ...
    if (!d.publicPosts || !d.publicPosts.length) {
        container.innerHTML = `<div class="bb-empty-hint"><div style="font-size: calc(28px * var(--font-scale));margin-bottom:8px;">💭</div><div>等待爱豆发帖中...</div><div style="font-size: calc(11px * var(--font-scale));color:#ddd;margin-top:4px;">点击右上角铃铛催更</div></div>`;
        return;
    }
...
function bbRenderPostCard(aiId, post) {
    ...
    const limitHint = charLimit < 999 ? `<div style="font-size: calc(10px * var(--font-scale));color:#f59e0b;margin-bottom:4px;">⚠️ 当前字数限制 ${charLimit} 字</div>` : '';
    ...
    <div class="bb-public-post" id="post-${post.id}" data-post-id="${post.id}">
        <div class="bb-post-header">
            ...
            ${unreadDot}
            <button onclick="bbCallApiForView('public')" style="background:none;border:none;cursor:pointer;color:#bbb;font-size: calc(14px * var(--font-scale));padding:4px 6px;" title="催更"><i class="fas fa-sync-alt"></i></button>
        </div>
...
function bbShowSecretRejectDialog(aiId, d, s, name) {
    ...
    <div style="background:#fff;border-radius:24px;padding:24px;max-width:280px;width:90%;text-align:center;box-shadow:0 20px 40px rgba(0,0,0,0.12);animation:bbPopIn 0.3s ease;">
        <div style="font-size: calc(32px * var(--font-scale));margin-bottom:12px;">🔒</div>
        <div style="font-size: calc(14px * var(--font-scale));font-weight:700;color:#333;margin-bottom:8px;">私信未解锁</div>
        <div style="font-size: calc(13px * var(--font-scale));color:#777;line-height:1.6;margin-bottom:16px;">${msg}</div>
        <div style="background:#f5f5f5;border-radius:12px;padding:10px;margin-bottom:16px;">
            <div style="font-size: calc(11px * var(--font-scale));color:#aaa;margin-bottom:4px;">亲密度进度</div>
            ...
            <div style="font-size: calc(10px * var(--font-scale));color:#aaa;margin-top:4px;">${progress} / 100</div>
        </div>
...
function bbRenderSecretMessages(aiId) {
    ...
    container.innerHTML = '';
    if (!d.secretUnlocked) {
        container.innerHTML = `<div class="bb-secret-locked"><div style="font-size: calc(40px * var(--font-scale));margin-bottom:16px;">💌</div><div style="font-size: calc(14px * var(--font-scale));font-weight:700;color:#333;margin-bottom:8px;">专属私信通道</div><div style="font-size: calc(12px * var(--font-scale));color:#999;text-align:center;line-height:1.6;">长按爱豆头像可尝试进入</div></div>`;
        return;
    }
    const msgs = d.secretMessages || [];
    if (!msgs.length) {
        container.innerHTML = `<div class="bb-empty-hint"><div style="font-size: calc(24px * var(--font-scale));margin-bottom:8px;">💬</div><div style="font-size: calc(12px * var(--font-scale));">暂无私信</div></div>`;
        return;
    }
    msgs.forEach(msg => {
...
function bbRenderFanShop(aiId) {
    ...
    <div class="bb-shop-idol-header">
        ...
        <div style="flex:1;">
            <div style="font-size: calc(14px * var(--font-scale));font-weight:800;color:#111;">${name} 的官方周边</div>
            <div style="font-size: calc(11px * var(--font-scale));color:#aaa;">OFFICIAL MERCHANDISE</div>
        </div>
        <button onclick="bbCallApiForView('shop')" style="background:#f5f5f5;border:none;border-radius:12px;padding:6px 12px;font-size: calc(11px * var(--font-scale));cursor:pointer;color:#111;"><i class="fas fa-sync-alt"></i> 刷新</button>
    </div>`);

    const merch = d.merch || [];
    if (!merch.length) {
        container.insertAdjacentHTML('beforeend', `<div class="bb-empty-hint"><div style="font-size: calc(28px * var(--font-scale));margin-bottom:8px;">🛍️</div><div>暂无周边上架</div><div style="font-size: calc(11px * var(--font-scale));color:#ddd;margin-top:4px;">点击刷新获取最新周边</div></div>`);
        return;
    }
    merch.forEach(item => {
        ...
        <div class="bb-shop-item">
            <div class="bb-shop-info">
                ...
                <div style="font-size: calc(11px * var(--font-scale));color:#aaa;margin-top:2px;">${item.desc || '限量发售'}</div>
...
window.bbFanBuyItem = async function(aiId, itemId) {
    ...
    <div style="background:#fff; border-radius:20px; width:85%; max-width:280px; padding:0; text-align:center; overflow:hidden; box-sizing: border-box; border: 1px solid #eee;">
        <div style="padding:25px 20px 20px;">
            <div style="font-size: calc(15px * var(--font-scale)); font-weight:800; color:#111; margin-bottom:8px;">确认购买</div>
            <div style="font-size: calc(12px * var(--font-scale)); color:#666;">将消耗 <b style="color:#111;">${item.price}</b> 豆币购买<br>「${item.name}」</div>
        </div>
        <div style="display:flex; border-top:1px solid #eee; box-sizing: border-box;">
            <div onclick="this.closest('.modal-overlay').remove()" style="flex:1; padding:12px; cursor:pointer; color:#999; font-size: calc(13px * var(--font-scale)); font-weight:600;">取消</div>
            <div id="bb-confirm-buy-btn" style="flex:1; padding:12px; cursor:pointer; color:#111; font-weight:800; font-size: calc(13px * var(--font-scale)); border-left:1px solid #eee;">确定</div>
        </div>
    </div>`;
...
function bbShowPurchaseReceipt(item, aiId) {
    ...
    <div style="background:#fff;border-radius:24px;padding:30px 24px;max-width:300px;width:90%;text-align:center;box-shadow:0 20px 40px rgba(0,0,0,0.12);">
        <div style="font-size: calc(36px * var(--font-scale));margin-bottom:12px;">🎁</div>
        <div style="font-weight:900;font-size: calc(16px * var(--font-scale));color:#111;margin-bottom:4px;">购买成功！</div>
        <div style="background:#f9f9f9;border-radius:16px;padding:16px;text-align:left;margin-bottom:20px;">
            <div style="display:flex;justify-content:space-between;font-size: calc(12px * var(--font-scale));padding:6px 0;border-bottom:1px dashed #eee;"><span style="color:#888;">商品</span><span style="font-weight:700;color:#111;">${item.name}</span></div>
            <div style="display:flex;justify-content:space-between;font-size: calc(12px * var(--font-scale));padding:6px 0;border-bottom:1px dashed #eee;"><span style="color:#888;">来自</span><span style="font-weight:700;color:#111;">${name}</span></div>
            <div style="display:flex;justify-content:space-between;font-size: calc(12px * var(--font-scale));padding:6px 0;"><span style="color:#888;">花费</span><span style="font-weight:800;color:#111;">${item.price} 豆币</span></div>
        </div>
        <div style="font-size: calc(11px * var(--font-scale));color:#aaa;margin-bottom:16px;">📦 已存入我的背包</div>
...
function bbRenderCreatorHistory(aiId) {
    ...
    if (!posts.length) { container.innerHTML = '<div class="bb-empty-hint"><div style="font-size: calc(12px * var(--font-scale));">尚未发布任何动态</div></div>'; return; }
    posts.slice(0, 10).forEach(post => {
        ...
        <div class="bb-history-post" onclick="bbIdolViewHistoryFans('${aiId}', '${post.id}')" style="cursor:pointer; border:1px solid #f0f0f0; padding:12px; border-radius:12px; margin-bottom:10px; background:#fff; transition:0.2s; box-shadow:0 2px 8px rgba(0,0,0,0.02);">
            <div style="font-size: calc(13px * var(--font-scale));color:#333;margin-bottom:8px;">${post.content}</div>
            <div style="display:flex;justify-content:space-between;font-size: calc(11px * var(--font-scale));color:#aaa;"><span>${timeStr}</span><span>❤️ ${post.likes} · 💬 ${post.comments} <i class="fas fa-chevron-right" style="margin-left:4px; font-size: calc(9px * var(--font-scale));"></i></span></div>
        </div>`);
    });
}
...
function bbRenderFanWaterfall(aiId) {
    ...
    if (!fans.length) {
        container.innerHTML = `<div style="text-align:center;padding:30px;color:#bbb;"><div style="font-size: calc(28px * var(--font-scale));margin-bottom:8px;">💬</div><div style="font-size: calc(12px * var(--font-scale));">点击"发布动态"或刷新按钮生成粉丝反应</div></div>`;
        return;
    }
    fans.forEach(fan => {
        ...
        <div class="bb-fan-reply-card ${isAi ? 'bb-ai-persona-reply' : ''}">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
                ...
                <div style="flex:1;">
                    <div style="font-size: calc(13px * var(--font-scale));font-weight:700;color:${isAi ? '#111' : '#333'};">${fan.name} ${isAi ? '<span style="font-size: calc(9px * var(--font-scale));background:#111;color:#fff;padding:2px 6px;border-radius:6px;letter-spacing:1px;margin-left:4px;">SPECIAL</span>' : ''}</div>
                    <span style="font-size: calc(10px * var(--font-scale));color:#888;font-weight:600;">${fan.type}</span>
                </div>
                <button class="bb-reply-fan-btn" onclick="bbIdolReplyFan('${aiId}','${fan.id}','${fan.type}')">回复</button>
            </div>
            <div style="font-size: calc(13px * var(--font-scale));color:#444;line-height:1.5;">${fan.comment}</div>
...
window.bbIdolReplyFan = function(aiId, fanId, fanType) {
    ...
    <div class="bfr-input-row" style="display:flex;gap:6px;align-items:center;margin-top:8px;">
        <input type="text" id="bfr-input-${fanId}" style="flex:1;border:1px solid #ddd;border-radius:14px;padding:7px 12px;font-size: calc(12px * var(--font-scale));outline:none;background:#fff;min-width:0;" placeholder="输入回复内容...">
        <button onclick="bbIdolSubmitManualReply('${aiId}','${fanId}','${fanType}')" style="width:30px;height:30px;background:#111;color:#fff;border:none;border-radius:50%;cursor:pointer;flex-shrink:0;display:flex;align-items:center;justify-content:center;"><i class="fas fa-paper-plane" style="font-size: calc(10px * var(--font-scale));"></i></button>
    </div>`;
...
window.bbIdolSubmitManualReply = function(aiId, fanId, fanType) {
    ...
    const resultEl = document.getElementById(`fan-reply-result-${fanId}`);
    if (resultEl) {
        resultEl.innerHTML = `<div style="background:#f9f9f9;border-radius:10px;padding:8px 10px;margin-top:8px;font-size: calc(12px * var(--font-scale));border:1px solid #eee;"><div style="color:#333;font-weight:600;">我的回复：${reply}</div>${effect ? `<div style="color:#111;font-weight:700;margin-top:4px;">${effect}</div>` : ''}</div>`;
    }
    const fanCount = document.getElementById('bb-idol-fan-count');
    if (fanCount) fanCount.textContent = bbFormatNum(d.idolFanCount || 0);
    const revEl = document.getElementById('bb-idol-revenue');
...
window.bbIdolOpenFanChat = function(aiId, fanId, fanName, fanAvatar) {
    ...
    modal.innerHTML = `
        <div style="display:flex;align-items:center;gap:10px;padding:12px 16px 10px;border-bottom:1px solid #f5f5f5;flex-shrink:0;">
            ...
            <span style="font-size: calc(13px * var(--font-scale));font-weight:800;color:#111;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${fanName}</span>
            <i class="fas fa-times" onclick="document.getElementById('bb-fan-private-backdrop').remove();document.getElementById('bb-fan-private-modal').remove();" style="cursor:pointer;color:#aaa;font-size: calc(15px * var(--font-scale));padding:4px;flex-shrink:0;"></i>
        </div>
        <div id="bfp-messages" style="flex:1;overflow-y:auto;padding:10px 14px;min-height:0;display:flex;flex-direction:column;gap:8px;background:#fafafa;">
            <div style="text-align:center;font-size: calc(9px * var(--font-scale));color:#bbb;padding:6px;letter-spacing:1px;font-weight:600;">PRIVATE CHAT · ${fanName}</div>
        </div>
        <div style="display:flex;gap:8px;padding:8px 12px;padding-bottom:max(8px, env(safe-area-inset-bottom, 8px));border-top:1px solid #eee;flex-shrink:0;background:#fff;">
            <input type="text" id="bfp-input" style="flex:1;border:1px solid #eee;border-radius:18px;padding:8px 13px;font-size: calc(12px * var(--font-scale));outline:none;background:#fff;color:#111;min-width:0;" placeholder="给 ${fanName} 发私信...">
            <button onclick="bbIdolSendFanPrivate('${aiId}','${fanId}','${fanName}','${fanAvatar}')" style="width:32px;height:32px;background:#111;color:#fff;border:none;border-radius:50%;cursor:pointer;flex-shrink:0;display:flex;align-items:center;justify-content:center;"><i class="fas fa-paper-plane" style="font-size: calc(10px * var(--font-scale));"></i></button>
        </div>`;
...
function bbRenderIdolMerch(aiId) {
    ...
    container.innerHTML = '';
    const merch = d.merch || [];
    if (!merch.length) { container.innerHTML = '<div class="bb-empty-hint"><div style="font-size: calc(12px * var(--font-scale));color:#aaa;">尚未上架任何周边</div></div>'; return; }
    merch.forEach(item => {
        container.insertAdjacentHTML('beforeend', `
        <div class="bb-idol-merch-item">
            <div style="flex:1;">
                <div style="font-size: calc(14px * var(--font-scale));font-weight:800;color:#111;">${item.name}</div>
                <div style="font-size: calc(11px * var(--font-scale));color:#aaa;margin-top:2px;">${item.desc || '限量发售'}</div>
                <div style="font-size: calc(12px * var(--font-scale));color:#111;font-weight:800;margin-top:6px;">${item.price} 豆币 <span style="color:#aaa;font-weight:500;margin-left:8px;">已售 ${item.sales || 0}</span></div>
            </div>
...
window.bbIdolPromoteMerch = async function(aiId, itemId) {
    ...
    modal.className = 'modal-overlay active';
    modal.style.cssText = 'z-index:6500;';
    modal.innerHTML = `<div style="background:#fff;border-radius:24px;padding:30px 24px;max-width:300px;width:90%;text-align:center;"><div style="font-size: calc(32px * var(--font-scale));margin-bottom:12px;">🎤</div><div style="font-size: calc(13px * var(--font-scale));line-height:1.6;color:#555;margin-bottom:20px;font-style:italic;">「${speech.trim()}」</div><div style="font-size: calc(16px * var(--font-scale));font-weight:900;color:#111;margin-bottom:16px;">+${revenue} 豆币收益！</div><button onclick="this.closest('.modal-overlay').remove()" style="width:100%;padding:14px;background:#111;color:#fff;border:none;border-radius:14px;font-weight:800;cursor:pointer;">太棒了</button></div>`;
    document.body.appendChild(modal);
}
...
function bbRenderIdolPR(aiId) {
    ...
    if (!crises.length) {
        container.innerHTML = `<div style="display:flex;flex-direction:column;align-items:center;padding:60px 20px;text-align:center;"><div style="font-size: calc(32px * var(--font-scale));margin-bottom:12px;">🔥</div><div style="font-size: calc(16px * var(--font-scale));font-weight:800;color:#111;margin-bottom:8px;">公关热搜榜</div><div style="font-size: calc(12px * var(--font-scale));color:#aaa;line-height:1.6;margin-bottom:24px;">危机事件由系统定时触发<br>处理好危机可稳固粉丝基础</div><button onclick="bbIdolCheckNewCrisis('${aiId}')" style="padding:14px 24px;background:#111;color:#fff;border:none;border-radius:14px;font-weight:800;cursor:pointer;"><i class="fas fa-sync-alt"></i> 立即触发危机</button></div>`;
        return;
    }
    container.innerHTML = `<button onclick="bbCallApiForView('pr')" style="position:sticky;top:0;left:0;z-index:10;background:rgba(255,255,255,0.95);backdrop-filter:blur(10px);border:1px solid #eee;border-radius:14px;padding:10px 16px;font-size: calc(12px * var(--font-scale));font-weight:700;cursor:pointer;color:#111;margin-bottom:16px;display:block;"><i class="fas fa-sync-alt"></i> 触发新危机</button>`;
    crises.forEach(crisis => {
        const bgColor = crisis.resolved ? '#fff' : '#fff';
        const borderColor = crisis.resolved ? '#eee' : '#111';
        const statusTag = crisis.resolved ? `<span style="background:#f5f5f5;color:#555;font-size: calc(10px * var(--font-scale));padding:4px 8px;border-radius:6px;font-weight:700;">已处理 ${crisis.outcome}</span>` : `<span class="bb-crisis-badge">🔥 上热搜了</span>`;
        const comments = (crisis.hotComments || []).map(c => {
            const bgMap = { support: '#fafafa', against: '#f5f5f5', neutral: '#fff' };
            return `<div class="bb-hot-comment ${c.stance}" style="background:${bgMap[c.stance]||'#fafafa'}; border:1px solid #eee;"><span style="font-size: calc(11px * var(--font-scale));font-weight:700;color:#111;">${c.user}</span><span style="font-size: calc(12px * var(--font-scale));color:#555;margin-left:8px;">${c.text}</span></div>`;
        }).join('');
        const prForm = !crisis.resolved ? `<div style="margin-top:16px;background:#fafafa;border-radius:16px;padding:16px;border:1px solid #eee;"><div style="font-size: calc(11px * var(--font-scale));font-weight:800;color:#111;margin-bottom:8px;">⚡ 发布公关声明</div><textarea id="pr-stmt-${crisis.id}" rows="3" style="width:100%;border:1px solid #ddd;border-radius:12px;padding:10px;font-size: calc(12px * var(--font-scale));resize:none;outline:none;" placeholder="诚恳回应，解释事情经过..."></textarea><button onclick="bbIdolSubmitPR('${aiId}','${crisis.id}')" style="margin-top:10px;width:100%;padding:12px;background:#111;color:#fff;border:none;border-radius:12px;font-size: calc(12px * var(--font-scale));font-weight:800;cursor:pointer;">发布声明</button></div>` : `<div style="margin-top:12px;background:#f9f9f9;border-radius:12px;padding:12px;font-size: calc(12px * var(--font-scale));color:#333;border:1px solid #eee;">声明内容：${crisis.statement || ''}</div>`;
        
        container.insertAdjacentHTML('beforeend', `<div class="bb-crisis-board" style="margin-bottom:20px;"><div class="bb-crisis-header" style="background:${bgColor};border-color:${borderColor};">${statusTag}<div class="bb-crisis-title">${crisis.title}</div><div style="font-size: calc(12px * var(--font-scale));color:#666;line-height:1.5;">${crisis.desc}</div>${!crisis.resolved ? `<div style="font-size: calc(11px * var(--font-scale));color:#ff4d4f;font-weight:800;margin-top:8px;">豆币每秒损失中 -${crisis.drainRate||1} 豆币/秒</div>` : ''}</div><div style="display:flex;flex-direction:column;gap:6px;margin-top:8px;">${comments}</div>${prForm}</div>`);
...
function bbShowCrisisAlert(crisis) {
    ...
    alert.innerHTML = `<div class="bb-crisis-alert-inner"><div class="bb-crisis-badge" style="background:#111; color:#fff;">🔥 紧急通知</div><div style="font-size: calc(16px * var(--font-scale));font-weight:800;color:#111;margin-bottom:4px;">${crisis.title}</div><div style="font-size: calc(12px * var(--font-scale));color:#666;">${crisis.desc.slice(0, 60)}...</div><div style="margin-top:16px;display:flex;gap:10px;"><button onclick="this.closest('.bb-crisis-alert').remove(); bbSwitchIdolTab('pr');" style="flex:1;padding:12px;background:#111;color:#fff;border:none;border-radius:14px;font-weight:800;cursor:pointer;">立即处理</button><button onclick="this.closest('.bb-crisis-alert').remove()" style="flex:1;padding:12px;background:#f5f5f5;border:none;border-radius:14px;cursor:pointer;font-weight:600;">稍后</button></div></div>`;
...
window.bbIdolSubmitPR = async function(aiId, crisisId) {
    ...
    modal.className = 'modal-overlay active';
    modal.style.cssText = 'z-index:6500;';
    modal.innerHTML = `<div style="background:#fff;border-radius:24px;padding:30px 24px;max-width:300px;width:90%;text-align:center;"><div style="font-size: calc(36px * var(--font-scale));margin-bottom:12px;">${sincerity === '诚恳' ? '🌟' : sincerity === '一般' ? '😐' : '💀'}</div><div style="font-size: calc(16px * var(--font-scale));font-weight:900;color:#111;margin-bottom:8px;">公关结果</div><div style="font-size: calc(12px * var(--font-scale));color:${outcomeColor};font-weight:800;background:#f9f9f9;padding:10px 16px;border-radius:12px;margin:16px 0;">${sincerity} · ${ending}</div><div style="font-size: calc(12px * var(--font-scale));color:#666;margin:12px 0;">粉丝 ${fanChange >= 0 ? '+' : ''}${bbFormatNum(fanChange)} · 收益 ${incomeChange >= 0 ? '+' : ''}${incomeChange}</div><button onclick="this.closest('.modal-overlay').remove();bbRenderIdolPR('${aiId}');" style="width:100%;padding:14px;background:#111;color:#fff;border:none;border-radius:14px;font-weight:800;cursor:pointer;margin-top:10px;">确认结果</button></div>`;
    document.body.appendChild(modal);
    bbRenderIdolPR(aiId);
}
...
function bbRenderBackpack() {
    ...
    container.innerHTML = '';
    if(!items.length) {
        container.innerHTML = `<div class="bb-empty-hint"><div style="font-size: calc(32px * var(--font-scale));margin-bottom:8px;">🛍️</div><div>背包空空的</div><div style="font-size: calc(11px * var(--font-scale));color:#ddd;margin-top:4px;">去 Bubble 周边商城购买商品吧</div></div>`;
        return;
    }
    ...
    items.reverse().forEach(item => {
        ...
        container.insertAdjacentHTML('beforeend', `
            <div class="bb-backpack-item">
                <div style="width:40px;height:40px;background:#f5f5f5;border-radius:12px;display:flex;justify-content:center;align-items:center;font-size: calc(20px * var(--font-scale));">🎁</div>
                <div style="flex:1;">
                    <div style="font-size: calc(14px * var(--font-scale));font-weight:800;color:#111;">${item.name}</div>
                    <div style="font-size: calc(11px * var(--font-scale));color:#888;margin-top:2px;">来自 ${item.from || '未知'} · 花费 ${item.price} 豆币</div>
                </div>
                <div style="font-size: calc(10px * var(--font-scale));color:#ccc;">${timeStr}</div>
            </div>
        `);
    });
}
