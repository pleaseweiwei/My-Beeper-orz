...
function renderBillList() {
    ...
    if(payData.transactions.length === 0) {
        list.innerHTML = '<div style="text-align:center; padding:60px 20px; color:#aaa; font-size: calc(12px * var(--font-scale));"><i class="fas fa-receipt" style="font-size: calc(32px * var(--font-scale)); color:#eee; margin-bottom:10px; display:block;"></i>没有任何账单记录</div>';
        return;
    }
    ...
    if(res) {
        try {
            ...
            if(data.balance !== undefined && data.bankCard !== undefined) {
                ...
                savePayData();
                showKAlert(`<b style="color:#07c160; font-size: calc(16px * var(--font-scale));">测算完成！</b><br><br>根据你的设定...<br>你的零钱为：<b>¥${payData.balance}</b><br>银行存款为：<b>¥${payData.bankCard}</b><br><br>努力生活吧！`);
            }
        } catch(e) {
...
window.addBalancePrompt = function() {
    showKPrompt("增加余额", "你要凭空为自己增加多少零钱？", "输入金额 (如: 1000)", (val) => {
        ...
        // 2. 中额挑战 (1000 - 10000): 简单的抢金币游戏
            ...
            showKAlert(`<b style="font-size: calc(16px * var(--font-scale));">小试牛刀 (๑•̀ㅂ•́)و✧</b><br><br>金额有点多，玩个简单的小游戏证明你的手速！<br>目标：10秒收集 10 个金币。`, () => {
        ...
        // 3. 大额验证 (10000 - 100000): 【新游戏】智力数学题
            ...
            showKAlert(`<b style="font-size: calc(16px * var(--font-scale));">大额验证 (⊙_⊙)?</b><br><br>金额较大（1万-10万），需要进行智力验证！<br>请口算或心算：<br><br><b style="font-size: calc(18px * var(--font-scale));">${n1} × ${n2} + ${n3} = ?</b>`, () => {
                // 延迟一点点打开输入框，防止弹窗冲突
                setTimeout(() => {
        ...
        // 4. 巨额考验 (>= 100000): 困难版抢金币
            ...
            showKAlert(`<b style="font-size: calc(16px * var(--font-scale));">巨款预警 Σ(っ °Д °;)っ</b><br><br>想要凭空拿十万以上，必须通过【地狱级】考验！<br>金币会乱飞且消失得很快，准备好了吗？`, () => {
...
function createFloatingScore(x, y, container) {
    ...
    float.style.cssText = `
        ...
        color: #07c160; font-weight: 900; font-size: calc(24px * var(--font-scale));
...
function endCoinGame(score, target, amount) {
    ...
    setTimeout(() => {
        ...
        if(score >= target) {
            ...
            savePayData();
            setTimeout(() => {
                showKAlert(`<b style="font-size: calc(16px * var(--font-scale)); color:#07c160;">挑战成功！(oﾟvﾟ)ノ</b><br><br>你不仅手速惊人，还抢到了 <b>${score}</b> 个金币！<br><br>【¥${amount.toFixed(2)}】 已存入你的钱包！`);
            }, 300);
        } else {
            setTimeout(() => {
                showKAlert(`<b style="font-size: calc(16px * var(--font-scale)); color:#ff4d4f;">挑战失败...(；′⌒\`)</b><br><br>只收集了 ${score} 个金币，距离目标 ${target} 个还差一点。<br>横财从指缝中溜走了~`);
            }, 300);
        }
    }, 100);
}
...
function renderYuebaoPage() {
    ...
    if(profits.length === 0) {
        list.innerHTML = '<div style="color:#999; font-size: calc(12px * var(--font-scale));">暂无收益记录</div>';
    } else {
        list.innerHTML = profits.map(t => `
            <div style="display:flex; justify-content:space-between; border-bottom:1px solid #eee; padding-bottom:10px;">
                <span style="font-size: calc(12px * var(--font-scale));">${new Date(t.time).toLocaleDateString()}</span>
...
function renderIntimatePage() {
    ...
    if(binds.length > 0) {
        html += `<div style="font-size: calc(12px * var(--font-scale)); color:#999; margin:10px 0 10px;">我为TA开通的亲密付</div>`;
        binds.forEach(id => {
                ...
                <div class="intimate-card" style="background:#fff; border-radius:16px; padding:15px; box-shadow:0 4px 15px rgba(0,0,0,0.02); border:1px solid #f0f0f0; margin-bottom:10px;">
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        <div style="display:flex; align-items:center; gap:10px;">
                            ...
                            <div>
                                <div style="font-weight:700; font-size: calc(14px * var(--font-scale)); color:#333;">${f.remark || f.realName}</div>
                                <div style="font-size: calc(10px * var(--font-scale)); color:#999;">本月已花: ¥${info.spent.toFixed(2)}</div>
                            </div>
                        </div>
                        <div style="text-align:right;">
                            <div style="font-size: calc(10px * var(--font-scale)); color:#888;">剩余额度</div>
                            <div style="font-size: calc(14px * var(--font-scale)); font-weight:700; color:#2b2b2b;">${remain === '无限' ? '无限额度' : '¥' + remain}</div>
                        </div>
                    </div>
                    <div style="display:flex; gap:10px; border-top:1px solid #f0f0f0; padding-top:10px; margin-top:10px;">
                        <button class="btn-secondary" style="flex:1; height:30px; font-size: calc(11px * var(--font-scale));" onclick="unbindIntimate('${id}')">解除绑定</button>
                    </div>
                </div>
            `;
        });
    ...
    if(bindsFrom.length > 0) {
        html += `<div style="font-size: calc(12px * var(--font-scale)); color:#999; margin:20px 0 10px;">TA为我开通的亲密付</div>`;
    ...
    <div class="intimate-card" style="background:#fff; border-radius:16px; padding:15px; box-shadow:0 4px 15px rgba(0,0,0,0.02); border:1px solid #f0f0f0; margin-bottom:10px;">
        <div style="display:flex; justify-content:space-between; align-items:center;">
            <div style="display:flex; align-items:center; gap:10px;">
                ...
                <div>
                    <div style="font-weight:700; font-size: calc(14px * var(--font-scale)); color:#333;">${f.remark || f.realName}</div>
                    <div style="font-size: calc(10px * var(--font-scale)); color:#999;">我已消费: ¥${info.spent.toFixed(2)}</div>
                </div>
            </div>
            <div style="text-align:right;">
                <div style="font-size: calc(10px * var(--font-scale)); color:#888;">可用额度</div>
                <div style="font-size: calc(14px * var(--font-scale)); font-weight:700; color:#07c160;">${remain === '无限' ? '无限额度' : '¥' + remain}</div>
            </div>
        </div>

        <div style="display:flex; gap:10px; border-top:1px solid #f0f0f0; padding-top:10px; margin-top:10px;">
            <button class="btn-secondary" style="flex:1; height:30px; font-size: calc(11px * var(--font-scale));" onclick="unbindIntimateFrom('${id}')">解绑</button>
        </div>
    </div>
`;
    ...
    if(html === '') {
        html = '<div style="text-align:center; color:#999; font-size: calc(12px * var(--font-scale)); padding:40px 20px;">暂无亲密付记录</div>';
    }
...
function finishJob(success, reward) {
    ...
    if (success) {
        ...
        savePayData();
        showKAlert(`<b style="color:#111; font-size: calc(16px * var(--font-scale));">辛苦啦，打工人！(๑•̀ㅂ•́)و✧</b><br><br>老板很满意你的表现，给你发了工资：<br><br><b style="font-size: calc(24px * var(--font-scale)); color:#111;">¥${reward}</b><br><br>去搓顿好的吧！`, quitJob);
    } else {
        showKAlert(`<b style="color:#ff4d4f; font-size: calc(16px * var(--font-scale));">搞砸啦！(；′⌒\`)</b><br><br>老板看着一团糟的现场，把你扫地出门了。<br>白干啦，没钱拿~<br>下次再来吧！`, quitJob);
    }
}
...
function g2NextOrder() {
    ...
    document.getElementById('g2-order-text').innerHTML = `💬 ${g}<b>${r.name}</b>！<br><span style="font-size: calc(11px * var(--font-scale)); color:#888;">配方：${r.emoji}</span>`;
}
...
window.openIdolFile = function(clickedElement) {
    ...
    setTimeout(() => {
        ...
        if (isWin) {
            ...
            showKAlert(`<b style="color:#111; font-size: calc(18px * var(--font-scale));">恭喜制作人！👑</b><br><br>${successMsgs}<br><br>狂赚 <b style="color:#111; font-size: calc(24px * var(--font-scale));">¥${winAmount.toFixed(2)}</b>`);
            ...
            list.insertAdjacentHTML('afterbegin', `
                <div style="background:#fff; border-radius:12px; padding:12px 15px; display:flex; justify-content:space-between; border:1px solid #111; box-shadow:4px 4px 0 rgba(0,0,0,1);">
                    <div>
                        <div style="font-size: calc(13px * var(--font-scale)); font-weight:800; color:#111;">爆红出道 (${multiplier}x)</div>
                        <div style="font-size: calc(10px * var(--font-scale)); color:#888; margin-top:4px;">投资: ¥${currentIdolInvestment.toFixed(2)}</div>
                    </div>
                    <div style="color:#111; font-weight:800; font-size: calc(16px * var(--font-scale));">+${(winAmount - currentIdolInvestment).toFixed(2)}</div>
                </div>
            `);
        } else {
            ...
            showKAlert(`<b style="color:#555; font-size: calc(18px * var(--font-scale));">投资血本无归 💔</b><br><br><span style="font-size: calc(13px * var(--font-scale)); color:#444; line-height:1.6;">${failMsg}</span><br><br><span style="color:#aaa; font-size: calc(11px * var(--font-scale));">你投入的 ¥${currentIdolInvestment.toFixed(2)} 打水漂了，下次擦亮眼睛吧。</span>`);
            ...
            list.insertAdjacentHTML('afterbegin', `
                <div style="background:#f9f9f9; border-radius:12px; padding:12px 15px; display:flex; justify-content:space-between; border:1px solid #eee;">
                    <div>
                        <div style="font-size: calc(13px * var(--font-scale)); font-weight:700; color:#666;">素人跑路</div>
                        <div style="font-size: calc(10px * var(--font-scale)); color:#999; margin-top:4px;">颗粒无收</div>
                    </div>
                    <div style="color:#aaa; font-weight:700; font-size: calc(16px * var(--font-scale));">-${currentIdolInvestment.toFixed(2)}</div>
                </div>
            `);
        }
