...
function renderForumFeed() {
    ...
    if (posts.length === 0) {
        listEl.innerHTML = `<div style="text-align:center; padding:60px 20px; color:#aaa; font-size: calc(12px * var(--font-scale));">这里空空如也，来发第一篇帖子吧 ☁️</div>`;
        return;
    }
...
function renderForumMyPosts() {
    ...
    if (posts.length === 0) {
        listEl.innerHTML = `<div style="text-align:center; padding:40px; color:#aaa; font-size: calc(12px * var(--font-scale));">暂无发布内容</div>`;
        return;
    }
...
async function renderForumHotSearch() {
    ...
    listEl.innerHTML = `<div style="text-align:center; padding:40px; color:#aaa; font-size: calc(12px * var(--font-scale));"><i class="fas fa-spinner fa-spin"></i> 正在生成实时热搜榜...</div>`;
...
`;
    ...
    try {
        ...
        listEl.innerHTML = `<div style="text-align:center; padding:40px; color:#aaa; font-size: calc(12px * var(--font-scale));">生成热搜榜失败了，再试一次吧</div>`;
    }
}
...
window.forumGeneratePostsByHot = async function(keyword) {
    const listEl = document.getElementById('forum-feed-list');
    if (!listEl) return;
    listEl.innerHTML = `<div style="text-align:center; padding:40px; color:#aaa; font-size: calc(12px * var(--font-scale));"><i class="fas fa-spinner fa-spin"></i> 正在加载关于「${keyword}」的讨论...</div>`;
...
`;
    ...
    try {
        ...
        if (jsonMatch) {
            ...
            let newPostsHtml = `<div style="padding:0 24px 10px; font-weight:800; font-size: calc(16px * var(--font-scale));">🔍 关于「${keyword}」</div>`;
        ...
        listEl.innerHTML = `<div style="text-align:center; padding:40px; color:#aaa; font-size: calc(12px * var(--font-scale));">加载失败，请重试</div>`;
    }
}
...
function renderForumComments(postId) {
    ...
    if (!post.comments || post.comments.length === 0) {
        commentsEl.innerHTML = `<div style="text-align:center; padding:20px; color:#aaa; font-size: calc(12px * var(--font-scale));">暂无评论，来抢沙发吧</div>`;
        return;
    }
...
function renderForumInbox() {
    ...
    if (!forumData.inbox || forumData.inbox.length === 0) {
        listEl.innerHTML = `<div style="text-align:center; padding:40px; color:#ccc; font-size: calc(12px * var(--font-scale));">暂无新消息</div>`;
        return;
    }
...
window.openForumUserProfile = function(name, avatar) {
    ...
    if (userPosts.length > 0) {
        ...
        listEl.innerHTML = `<div style="text-align:center; padding:20px; color:#aaa; font-size: calc(12px * var(--font-scale));">这个人很懒，什么都没发。</div>`;
    }
...
window.forumOpenFanGroup = function() {
    ...
    const messagesEl = document.getElementById('forum-chat-messages');
    messagesEl.innerHTML = `<div style="text-align:center; color:#aaa; font-size: calc(12px * var(--font-scale)); margin-bottom:15px;">你在群里发了条新消息，看看粉丝们什么反应吧</div>`;
                        ...
                        setTimeout(() => {
                            messagesEl.innerHTML += `
                                <div class="chat-row received" style="flex-direction:column; align-items:flex-start; margin-bottom:15px;">
                                    <div style="font-size: calc(10px * var(--font-scale)); color:#888; margin-left:46px; margin-bottom:4px;">${reply.name}</div>
...
window.forumGenerateRandomPosts = async function() {
    const listEl = document.getElementById('forum-feed-list');
    if(listEl) listEl.innerHTML = `<div style="text-align:center; padding:40px; color:#aaa; font-size: calc(12px * var(--font-scale));"><i class="fas fa-spinner fa-spin"></i> 正在召唤平行宇宙的网友发帖...</div>`;
...
async function generateInitialForumPosts() {
    ...
    if(listEl) listEl.innerHTML = `<div style="text-align:center; padding:40px; color:#aaa; font-size: calc(12px * var(--font-scale));"><i class="fas fa-spinner fa-spin"></i> 正在连接虚拟社区...</div>`;
